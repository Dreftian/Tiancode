...
> Note: the upstream skill ships optional Node harness scripts (recon, snap,
> eval) under `scripts/` that drive Playwright against live URLs. They are an
> evaluation convenience and are not bundled here; every design rule above works
> without them.

# Appendix: Override Stylesheets for Sites Without Source Access
