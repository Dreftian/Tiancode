---
name: i-have-adhd
description: Use ONLY when the user explicitly asks for ADHD-friendly output, or invokes /i-have-adhd. It shapes every later response for a reader with ADHD (next action first, numbered steps, state restated each turn, tangents deferred, concrete time estimates, visible wins) and stays on until the user says "stop adhd mode". Do not use for a general request to be brief, nor for ordinary formatting decisions.
...
Adapted from https://github.com/ayghri/i-have-adhd (MIT, (c) 2026 Ayoub Ghriss). See skills/LICENSE-AYGHRI.md.
