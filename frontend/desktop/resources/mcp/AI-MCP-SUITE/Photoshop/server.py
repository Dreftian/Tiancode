...
Controls Adobe Photoshop: opens documents, installs and runs scripts
(JSX/JSXBIN) via the scripts_dir, and talks to a UXP bridge over HTTP when
configured. Degrades gracefully when Photoshop or the bridge is missing.
"""
...
ALLOWED_EXTENSIONS = (".jsx", ".jsxbin", ".js")
DEFAULT_EXTENSION = ".jsx"

server = Server("photoshop", "1.0.0")
...
@server.tool(
    "open_document",
    "Open a document in Photoshop (spawns Photoshop; falls back to the UXP bridge).",
    {
        "type": "object",
...
def open_document(arguments):
    ...
    exe = CONFIG.get("photoshop_exe") or ""
    if os.path.isfile(exe):
        pid = spawn_detached([exe, path])
        return f"Started Photoshop (pid {pid}) opening: {path}"
    ...
    raise ToolError(
        ...
        "photoshop_exe not found and uxp_bridge_url not configured; set either in config.json / MCP_CONFIG",
    )


@server.tool(
    "install_script",
    "Install a script (.jsx/.jsxbin/.js) into the Photoshop scripts directory.",
    {
        "type": "object",
        "properties": {
            "name": {"type": "string", "description": "Script file name (e.g. export_layers.jsx)"},
...
@server.tool(
    "list_scripts",
    "List the scripts currently installed in the Photoshop scripts directory.",
...
def diagnostics(arguments):
    ...
    bridge = CONFIG.get("uxp_bridge_url") or ""
    lines = [
        f"server: photoshop v{server.version}",
        f"platform: {sys.platform}",
        f"photoshop_exe: {exe} ({'FOUND' if os.path.isfile(exe) else 'not found'})",
    ...
    if not os.path.isfile(exe):
        hints.append("install Photoshop or set photoshop_exe in config.json / MCP_CONFIG")
    if not os.path.isdir(scripts_dir):
