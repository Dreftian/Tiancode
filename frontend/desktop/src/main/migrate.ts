...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.tiancode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `tiancode.settings.dat` is special: it maps to the `tiancode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"tiancode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // tiancode.settings.dat → the electron settings store ("tiancode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "tiancode.global.dat").
  const storeName = filename === "tiancode.settings.dat" ? "tiancode.settings" : filename
