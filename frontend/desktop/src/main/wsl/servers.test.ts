...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "Tiancode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Tiancode", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.tiancode/bin/tiancode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, tiancodeChecks: {} })
})
...
test("ignores stale background Tiancode checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().tiancodeChecks).toEqual({})
})

test("ignores stale startup Tiancode checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Tiancode", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const tiancode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOpencode: async (distro) => {
      tiancode.push(distro)
      return "/home/me/.tiancode/bin/tiancode"
    },
  })
  ...
  expect(tiancode).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(tiancode).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().tiancodeChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Tiancode in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const tiancode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(tiancode).toEqual(["Debian"])
  expect(Object.keys(controller.getState().tiancodeChecks)).toEqual(["Debian"])
})
