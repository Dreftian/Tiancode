    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.TIANCODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "tiancode:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__TIANCODE__ ??= {}
  const pending = window.__TIANCODE__.deepLinks ?? []
  window.__TIANCODE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `tiancode.desktop.window.${windowID}.last-active-url`
}
...
const createPlatform = (windowState: DesktopWindowState): Platform => {
  ...
  return {
    ...
    setLoginItem(enabled) {
      return window.api.setLoginItem(enabled)
    },

    getLoginItem() {
      return window.api.getLoginItem()
    },

    backupNow() {
      return window.api.backup.now()
    },

    listBackups() {
      return window.api.backup.list()
    },
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("tiancode.global.dat").getItem("language")
