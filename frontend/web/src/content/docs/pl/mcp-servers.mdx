...
Możesz dodać dodatkowe informacje do tiancode za pomocą _Model Context Protocol_, w skrócie MCP. tiancode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [tiancode Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/tiancode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
tiancode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, tiancode:
...
Jeśli serwer wymaga uwierzytelnienia, tiancode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `tiancode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji tiancode bezpieczne przechowa tokeny w `~/.local/share/tiancode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w tiancode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie tiancode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć tiancode z kontem Sentry.
