---
title: Serwer
description: Interakcja z serwerem tiancode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `tiancode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient tiancode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `tiancode.local` |
...
Ustaw `TIANCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `tiancode` lub ustaw `TIANCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `tiancode serve`, jak i `tiancode web`.
...
Kiedy uruchomisz `tiancode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera tiancode do programowej interakcji z tiancode.
:::

Ta architektura umożliwia obsługę wielu klientów przez tiancode i programową interakcję z kodem otwartym.

Możesz uruchomić `tiancode serve`, aby uruchomić samodzielny serwer. Jeśli masz
tiancode TUI działa, `tiancode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki tiancode [IDE](/docs/ide).
...
Serwer tiancode udostępnia następujące interfejsy API.
