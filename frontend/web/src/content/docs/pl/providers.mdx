---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w tiancode.
---

import config from "../../../../config.mjs"
export const console = config.console

tiancode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji tiancode.
...
w `~/.local/share/tiancode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim tiancode
config.
...
Tiancode Zen to lista modeli dostarczonych przez zespół tiancode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z tiancode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Tiancode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Tiancode Zen` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Działa jak każdy inny dostawca w tiancode i jest całkowicie opcjonalny w użyciu.

---

## Tiancode Go

Tiancode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół tiancode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z tiancode.

1. Uruchom polecenie `/connect` w TUI, wybierz `Tiancode Go` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Aby używać Amazon Bedrock z tiancode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania tiancode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `tiancode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w tiancode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować tiancode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod tiancode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji tiancode.
...
tiancode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować tiancode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`tiancode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem tiancode:
...
Aby używać Google Vertex AI z tiancode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania tiancode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą tiancode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [tiancode-helicone-session](https://github.com/H2Shami/tiancode-helicone-session), aby automatycznie rejestrować każdą konwersację tiancode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja tiancode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować tiancode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować tiancode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować tiancode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem tiancode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/tiancode).
:::
...
Aby korzystać z Ollama Cloud z tiancode:
...
3. Skopiuj klucz API do użycia w tiancode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w tiancode musisz pobrać informacje o modelu lokalnie:
...
Tiancode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół tiancode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Tiancode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Tiancode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API tiancode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji tiancode.
...
5. Możesz je także dostosować za pomocą konfiguracji tiancode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z tiancode:
...
Możesz także dostosować modele za pomocą konfiguracji tiancode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z tiancode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `tiancode.json` w katalogu projektu:
...
Pola `limit` pozwalają tiancode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `tiancode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację tiancode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji tiancode.
