...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Tiancode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Tiancode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `TIANCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `TIANCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Tiancode lub gdy zmienna środowiskowa `TIANCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Tiancode:
