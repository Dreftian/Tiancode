...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) tiancode, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.tiancode/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/tiancode/tools/`.
