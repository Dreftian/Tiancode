...
Dzięki tiancode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie tiancode używa naszego własnego motywu `tiancode`.
...
tiancode ma kilka wbudowanych motywów.
...
- Chcą, aby Tiancode pasował do wyglądu ich terminala
...
tiancode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/tiancode/themes/*.json` lub `$XDG_CONFIG_HOME/tiancode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.tiancode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.tiancode/themes/*.json`
