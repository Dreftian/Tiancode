---
title: Zen
description: Wyselekcjonowana lista modeli dostarczonych przez Tiancode.
---
...
Tiancode Zen to lista przetestowanych i zweryfikowanych modeli udostępniana przez zespół Tiancode.

Zen działa jak każdy inny dostawca w Tiancode. Logujesz się do Tiancode Zen i otrzymujesz
...
:::tip
Przetestowaliśmy wybraną grupę modeli i dostawców, którzy dobrze współpracują z Tiancode.
:::
...
Tiancode Zen to brama AI, która daje dostęp do tych modeli.

---

## Jak to działa

Tiancode Zen działa jak każdy inny dostawca w Tiancode.

1. Logujesz się do **<a href={console}>Tiancode Zen</a>**, dodajesz dane
   rozliczeniowe i kopiujesz swój klucz API.
2. Uruchamiasz polecenie `/connect` w TUI, wybierasz Tiancode Zen i wklejasz swój klucz API.
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji Tiancode używa formatu
`tiancode/<model-id>`. Na przykład dla GPT 5.5 użyjesz w konfiguracji
`tiancode/gpt-5.5`.
...
W historii użycia możesz zauważyć [niedrogie modele](/docs/config/#models), takie jak Haiku, Nano lub Flash. Tiancode używa tych modeli do generowania tytułów sesji.
...
- DeepSeek V4 Flash Free jest dostępny w Tiancode przez ograniczony czas. Zespół wykorzystuje ten czas do zbierania opinii i ulepszania modelu.
- MiMo-V2.5 Free jest dostępny w Tiancode przez ograniczony czas. Zespół wykorzystuje ten czas do zbierania opinii i ulepszania modelu.
- Laguna S 2.1 Free jest dostępny w Tiancode przez ograniczony czas. Zespół wykorzystuje ten czas do zbierania opinii i ulepszania modelu.
- Ling-3.0-flash Free jest dostępny w Tiancode przez ograniczony czas. Zespół wykorzystuje ten czas do zbierania opinii i ulepszania modelu.
- LongCat-2.0 Free jest dostępny w Tiancode przez ograniczony czas. Zespół wykorzystuje ten czas do zbierania opinii i ulepszania modelu.
- North Mini Code Free jest dostępny w Tiancode przez ograniczony czas. Zespół wykorzystuje ten czas do zbierania opinii i ulepszania modelu.
- Nemotron 3 Ultra Free jest dostępny w Tiancode przez ograniczony czas. Zespół wykorzystuje ten czas do zbierania opinii i ulepszania modelu.
- Big Pickle to stealth model, który jest darmowy w Tiancode przez ograniczony czas. Zespół wykorzystuje ten czas do zbierania opinii i ulepszania modelu.
...
4. Nie tworzyć **lock-in**, pozwalając używać tego z dowolnym innym agentem kodującym. I zawsze pozwalać Ci używać w Tiancode także dowolnego innego dostawcy.
