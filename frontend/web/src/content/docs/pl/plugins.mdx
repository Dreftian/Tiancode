---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć tiancode.
---

Wtyczki umożliwiające wyłączenie tiancode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania tiancode.
...
- `.tiancode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/tiancode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/tiancode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/tiancode/tiancode.json`)
2. Konfiguracja projektu (`tiancode.json`)
3. Globalny katalog wtyczek (`~/.config/tiancode/plugins/`)
4. Katalog wtyczek projektu (`.tiancode/plugins/`)
...
tiancode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z tiancode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do tiancode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej tiancode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj tiancode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do tiancode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod tiancode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
