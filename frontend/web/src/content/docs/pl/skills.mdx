...
Umiejętności agenta pozwalają tiancode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
tiancode przeszukuje te lokalizacje:

- Project config: `.tiancode/skills/<name>/SKILL.md`
- Global config: `~/.config/tiancode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu tiancode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.tiancode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/tiancode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.tiancode/skills/git-release/SKILL.md` w ten sposób:
...
tiancode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `tiancode.json`:
...
**Dla agentów wbudowanych** (w `tiancode.json`):
