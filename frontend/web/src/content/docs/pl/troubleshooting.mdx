...
Aby debugować problemy z tiancode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\tiancode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `tiancode --log-level DEBUG`.

---

## Przechowywanie danych

tiancode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/tiancode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\tiancode`
...
tiancode Desktop uruchamia w tle lokalny serwer tiancode (sidecar `tiancode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Tiancode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/tiancode/tiancode.jsonc` (lub `~/.config/tiancode/tiancode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/tiancode/tiancode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\tiancode\tiancode.jsonc`
...
tiancode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/tiancode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\tiancode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby tiancode mógł ją odbudować.

1. Całkowicie zamknij tiancode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/tiancode`
- **Linux**: usuń `~/.cache/tiancode` (lub uruchom `rm -rf ~/.cache/tiancode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\tiancode`

3. Uruchom ponownie tiancode Desktop.

---

### Napraw problemy z połączeniem z serwerem

tiancode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `tiancode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `TIANCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `TIANCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows tiancode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami tiancode.

---

### Brak powiadomień

tiancode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla tiancode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie tiancode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji tiancode Desktop):

- `tiancode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `tiancode.global.dat` i `tiancode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z tiancode:
...
3. Upewnij się, że masz najnowszą wersję z `tiancode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `tiancode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\tiancode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. tiancode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\tiancode`

2. Uruchom ponownie kod tiancode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to tiancode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
tiancode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
