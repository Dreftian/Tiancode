...
With Tiancode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Tiancode uses our own `tiancode` theme.
...
Tiancode comes with several built-in themes.
...
- Want Tiancode to match their terminal's appearance
...
Tiancode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/tiancode/themes/*.json` or `$XDG_CONFIG_HOME/tiancode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.tiancode/themes/*.json`
4. **Current working directory** - Defined in `./.tiancode/themes/*.json`
