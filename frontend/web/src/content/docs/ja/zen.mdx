---
title: Tiancode Zen
description: Tiancode によって提供されるモデルの厳選されたリスト。
---
...
Tiancode Zen は、Tiancode チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は Tiancode のほかのプロバイダーと同じように動作します。Tiancode Zen にログインして API キーを取得できます。これは **完全にオプション** であり、Tiancode を使うために必須ではありません。
...
:::tip
Tiancode でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
Tiancode Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

Tiancode Zen は、Tiancode のほかのプロバイダーと同じように動作します。

1. **<a href={console}>Tiancode Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、Tiancode Zen を選んで API キーを貼り付けます。
...
Tiancode 設定で使う [model id](/docs/config/#models) は `tiancode/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `tiancode/gpt-5.5` を使用します。
...
使用履歴に Haiku、Nano、Flash などの[低コストモデル](/docs/config/#models)が表示されることがあります。Tiancode はこれらのモデルをセッションタイトルの生成に使用します。
...
- DeepSeek V4 Flash Free は期間限定で Tiancode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo-V2.5 Free は期間限定で Tiancode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Laguna S 2.1 Free は期間限定で Tiancode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Ling-3.0-flash Free は期間限定で Tiancode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- LongCat-2.0 Free は期間限定で Tiancode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- North Mini Code Free は期間限定で Tiancode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Ultra Free は期間限定で Tiancode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で Tiancode で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
私たちが Tiancode Zen を作った目的は次のとおりです。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに Tiancode では常にほかのプロバイダーも使えるようにすること。
