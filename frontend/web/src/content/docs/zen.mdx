---
title: Zen
description: Curated list of models provided by Tiancode.
---
...
Tiancode Zen is a list of tested and verified models provided by the Tiancode team.

Zen works like any other provider in Tiancode. You login to Tiancode Zen and get
...
:::tip
We tested a select group of models and providers that work well with Tiancode.
:::
...
Tiancode Zen is an AI gateway that gives you access to these models.

---

## How it works

Tiancode Zen works like any other provider in Tiancode.

1. You sign in to **<a href={console}>Tiancode Zen</a>**, add your billing
   details, and copy your API key.
2. You run the `/connect` command in the TUI, select Tiancode Zen, and paste your API key.
...
The [model id](/docs/config/#models) in your Tiancode config
uses the format `tiancode/<model-id>`. For example, for GPT 5.5, you would
use `tiancode/gpt-5.5` in your config.
...
You may notice [low-cost models](/docs/config/#models), such as Haiku, Nano, or Flash, in your usage history. Tiancode uses these models to generate session titles.
...
- DeepSeek V4 Flash Free is available on Tiancode for a limited time. The team is using this time to collect feedback and improve the model.
- MiMo-V2.5 Free is available on Tiancode for a limited time. The team is using this time to collect feedback and improve the model.
- Laguna S 2.1 Free is available on Tiancode for a limited time. The team is using this time to collect feedback and improve the model.
- Ling-3.0-flash Free is available on Tiancode for a limited time. The team is using this time to collect feedback and improve the model.
- LongCat-2.0 Free is available on Tiancode for a limited time. The team is using this time to collect feedback and improve the model.
- North Mini Code Free is available on Tiancode for a limited time. The team is using this time to collect feedback and improve the model.
- Nemotron 3 Ultra Free is available on Tiancode for a limited time. The team is using this time to collect feedback and improve the model.
- Big Pickle is a stealth model that's free on Tiancode for a limited time. The team is using this time to collect feedback and improve the model.
...
4. Have **no lock-in** by allowing you to use it with any other coding agent. And always let you use any other provider with Tiancode as well.
