...
Com o tiancode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o tiancode usa nosso próprio tema `tiancode`.
...
O tiancode vem com vários temas embutidos.
...
- Querem que o tiancode corresponda à aparência do seu terminal
...
O tiancode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/tiancode/themes/*.json` ou `$XDG_CONFIG_HOME/tiancode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.tiancode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.tiancode/themes/*.json`
