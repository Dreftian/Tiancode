---
title: TUI
description: Usando a interface de usuário de terminal tiancode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

O tiancode fornece uma interface de terminal interativa ou TUI para trabalhar em seus projetos com um LLM.

Executar tiancode inicia o TUI para o diretório atual.
...
Ao usar o TUI do tiancode, você pode digitar `/` seguido pelo nome de um comando para executar ações rapidamente. Por exemplo:
...
Adicione um provedor ao tiancode. Permite que você selecione entre os provedores disponíveis e adicione suas chaves de API.
...
Saia do tiancode. _Aliases_: `/quit`, `/q`
...
Isso é separado do `tiancode.json`, que configura o comportamento do servidor/runtime.
...
- `leader_timeout` - Controla por quanto tempo o Tiancode espera depois da leader key. O padrão é `2000`.
...
Use `TIANCODE_TUI_CONFIG` para carregar um caminho de configuração TUI personalizado.

### Attention

Attention permite que o TUI avise você quando o Tiancode espera uma resposta, precisa da aprovação de uma permissão, encontra um erro de sessão ou conclui uma sessão. Ative com `attention.enabled`; eventos integrados reproduzem sons quando acontecem. As notificações de desktop só são enviadas quando a janela do terminal não está em foco e não são usadas para eventos de subagent.
...
- `sound_pack` - ID do sound pack a ser usado. O padrão é `tiancode.default`.
