---
title: Plugins
description: Escreva seus próprios plugins para estender o tiancode.
---

Plugins permitem que você estenda o tiancode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do tiancode.
...
- `.tiancode/plugins/` - Plugins em nível de projeto
- `~/.config/tiancode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/tiancode/node_modules/`.
...
1. Configuração global (`~/.config/tiancode/tiancode.json`)
2. Configuração do projeto (`tiancode.json`)
3. Diretório de plugins global (`~/.config/tiancode/plugins/`)
4. Diretório de plugins do projeto (`.tiancode/plugins/`)
...
O tiancode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do tiancode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o tiancode.
...
:::note
Se você estiver usando o aplicativo desktop tiancode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o tiancode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao tiancode:
...
O helper `tool` cria uma ferramenta personalizada que o tiancode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o tiancode junto com as ferramentas integradas.
