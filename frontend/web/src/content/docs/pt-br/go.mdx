...
O Tiancode Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Tiancode. Você assina o Tiancode Go e
...
O Tiancode Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Tiancode Go funciona como qualquer outro provedor no Tiancode.

1. Você entra no **<a href={console}>Tiancode Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Tiancode Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Tiancode Go.
:::
...
O Tiancode Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Tiancode
usa o formato `tiancode-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`tiancode-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Tiancode também.
