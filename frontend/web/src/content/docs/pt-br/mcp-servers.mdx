...
Você pode adicionar ferramentas externas ao tiancode usando o _Model Context Protocol_, ou MCP. O tiancode suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do tiancode](https://opencode.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/tiancode`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O tiancode lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o tiancode irá:
...
Se o servidor requer autenticação, o tiancode solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `tiancode mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o tiancode armazenará os tokens de forma segura em `~/.local/share/tiancode/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no tiancode, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do tiancode como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o tiancode à sua conta do Sentry.
