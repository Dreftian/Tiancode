...
As habilidades do agente permitem que o tiancode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O tiancode pesquisa nesses locais:

- Configuração do projeto: `.tiancode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/tiancode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o tiancode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.tiancode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/tiancode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.tiancode/skills/git-release/SKILL.md` assim:
...
O tiancode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `tiancode.json`:
...
**Para agentes embutidos** (em `tiancode.json`):
