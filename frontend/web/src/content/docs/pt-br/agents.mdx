...
Existem dois tipos de agentes no tiancode; agentes primários e subagentes.
...
tiancode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Tiancode vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Tiancode vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Tiancode, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `tiancode.json`:
...
- Global: `~/.config/tiancode/agents/`
- Por projeto: `.tiancode/agents/`

```markdown title="~/.config/tiancode/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o tiancode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do tiancode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do tiancode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Tiancode Zen](/docs/zen), você usaria `tiancode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="tiancode.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="tiancode.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `tiancode models` para ver uma lista dos modelos disponíveis.
:::
...
```markdown title="~/.config/tiancode/agents/docs-writer.md"
---
...
```markdown title="~/.config/tiancode/agents/security-auditor.md"
---
