---
title: Servidor
description: Interaja com o servidor tiancode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `tiancode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente tiancode pode usar.
...
```bash
tiancode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `tiancode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
tiancode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `TIANCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `tiancode`, ou defina `TIANCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `tiancode serve` quanto ao `tiancode web`.

```bash
TIANCODE_SERVER_PASSWORD=your-password tiancode serve
```
...
Quando você executa `tiancode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor tiancode para interagir com o tiancode programaticamente.
:::

Essa arquitetura permite que o tiancode suporte múltiplos clientes e permite que você interaja com o tiancode programaticamente.

Você pode executar `tiancode serve` para iniciar um servidor autônomo. Se você tiver o TUI do tiancode em execução, `tiancode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do tiancode [IDE](/docs/ide).
...
O servidor tiancode expõe as seguintes APIs.
