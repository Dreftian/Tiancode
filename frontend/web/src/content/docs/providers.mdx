---
title: Providers
description: Using any LLM provider in Tiancode.
---

import config from "../../../config.mjs"
export const console = config.console

Tiancode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Tiancode config.
...
in `~/.local/share/tiancode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Tiancode
config.
...
Tiancode Zen is a list of models provided by the Tiancode team that have been
tested and verified to work well with Tiancode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Tiancode Zen.
:::

1. Run the `/connect` command in the TUI, select `Tiancode Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in Tiancode and is completely optional to use.

---

## Tiancode Go

Tiancode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Tiancode team that have been
tested and verified to work well with Tiancode.

1. Run the `/connect` command in the TUI, select `Tiancode Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with Tiancode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running tiancode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY tiancode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile tiancode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX tiancode
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `tiancode.json`:
...
Tiancode. Anthropic explicitly prohibits this.

Previous versions of Tiancode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Tiancode with zero setup:
...
You can configure tiancode to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="tiancode.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for tiancode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX tiancode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX tiancode
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your tiancode config.
...
Tiancode supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Tiancode uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Tiancode. Sign in and approve.

   :::note
   Tiancode requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Tiancode surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Tiancode forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Tiancode is experimental. Features, configuration, and
behavior may change in future releases.
:::

Tiancode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in tiancode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Tiancode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Tiancode
to only use your own GitLab-hosted instance, add the following to your
`tiancode.json` file. It is also recommended to disable session sharing.
...
Customize through `tiancode.json`:
...
When a `duo-workflow-*` model is selected, Tiancode will:
...
4. Route tool execution requests through Tiancode's permission-gated tool system
...
To use your GitHub Copilot subscription with tiancode:
...
To use GMI Cloud with Tiancode:
...
To use Google Vertex AI with Tiancode:
...
2. Set the required environment variables:
   ...
   Set them while running tiancode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id tiancode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through tiancode, you can always configure it yourself.
...
```jsonc title="~/.config/tiancode/tiancode.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [tiancode-helicone-session](https://github.com/H2Shami/tiancode-helicone-session) plugin to automatically log each Tiancode conversation as a session in Helicone.

```bash
npm install -g tiancode-helicone-session
```
...
```json title="tiancode.json"
{
  "plugin": ["tiancode-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Tiancode conversation listed as a separate session.
...
You can configure tiancode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="tiancode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure tiancode to use local models through LM Studio.

```json title="tiancode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure tiancode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Tiancode. See the [Ollama integration docs](https://docs.ollama.com/integrations/tiancode) for details.
:::

```json title="tiancode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Tiancode:
...
3. Copy the API key for use in Tiancode.
...
6. **Important**: Before using cloud models in Tiancode, you must pull the model information locally:
...
Tiancode Zen is a list of tested and verified models provided by the Tiancode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Tiancode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Tiancode Zen**.

   ```txt
   /connect
   ```

3. Enter your Tiancode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your tiancode config.
...
5. You can also customize them through your tiancode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' tiancode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group tiancode
   ```
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=tiancode&utm_medium=docs&utm_campaign=cortex-provider).

tiancode's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Tiancode captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting tiancode:
...
The model catalog is provided automatically. A minimal `tiancode.json` is all that's needed:
...
You can also customize models through your tiancode config. Here's an example of specifying provider routing order.
...
2. Select **SuperGrok Subscription**. Tiancode opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by Tiancode. Tiancode polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
Tiancode refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
:::tip
You can use any OpenAI-compatible provider with tiancode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in tiancode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `tiancode.json` file in your project directory:

   ```json title="tiancode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="tiancode.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Tiancode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `tiancode auth list` to see if the credentials
...
2. For custom providers, check the tiancode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your tiancode config.
