---
title: Complementos
description: Escriba sus propios complementos para extender Tiancode.
---

Los complementos le permiten extender Tiancode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Tiancode.
...
- `.tiancode/plugins/` - Complementos a nivel de proyecto
- `~/.config/tiancode/plugins/` - Complementos globales
...
```json title="tiancode.json"
{
  ...
  "plugin": ["tiancode-helicone-session", "tiancode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/tiancode/node_modules/`.
...
1. Configuración global (`~/.config/tiancode/tiancode.json`)
2. Configuración del proyecto (`tiancode.json`)
3. Directorio global de complementos (`~/.config/tiancode/plugins/`)
4. Directorio de complementos del proyecto (`.tiancode/plugins/`)
...
```json title=".tiancode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Tiancode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".tiancode/plugins/my-plugin.ts"
...
```js title=".tiancode/plugins/example.js"
...
- `client`: Un cliente SDK tiancode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar tiancode.
...
```js title=".tiancode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "tiancode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Tiancode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que tiancode lea archivos `.env`:

```javascript title=".tiancode/plugins/env-protection.js"
...
```javascript title=".tiancode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a tiancode:

```ts title=".tiancode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@tiancode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que tiancode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para tiancode junto con las herramientas integradas.
...
```ts title=".tiancode/plugins/compaction.ts"
import type { Plugin } from "@tiancode-ai/plugin"
...
```ts title=".tiancode/plugins/custom-compaction.ts"
import type { Plugin } from "@tiancode-ai/plugin"
