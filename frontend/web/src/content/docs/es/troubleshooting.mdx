...
Para depurar problemas con Tiancode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/tiancode/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\tiancode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `tiancode --log-level DEBUG`.

---

## Almacenamiento

tiancode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/tiancode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\tiancode`
...
Tiancode Desktop ejecuta un servidor Tiancode local (el sidecar `tiancode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Tiancode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/tiancode/tiancode.jsonc` (o `~/.config/tiancode/tiancode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/tiancode/tiancode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\tiancode\tiancode.jsonc`
...
Tiancode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/tiancode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\tiancode\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.tiancode/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Tiancode pueda reconstruirlo.

1. Salga de Tiancode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/tiancode`
- **Linux**: eliminar `~/.cache/tiancode` (o ejecutar `rm -rf ~/.cache/tiancode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\tiancode`

3. Reinicie el escritorio Tiancode.

---

### Solucionar problemas de conexión del servidor

Tiancode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `tiancode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `TIANCODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `TIANCODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Tiancode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Tiancode.

---

### Notificaciones que no se muestran

Tiancode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Tiancode en la configuración de su sistema operativo, y
...
1. Salga del escritorio Tiancode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Tiancode):

- `tiancode.settings.dat` (URL del servidor predeterminado de escritorio)
- `tiancode.global.dat` y `tiancode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Tiancode:
...
3. Asegúrese de tener la última versión con `tiancode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `tiancode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/tiancode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\tiancode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. tiancode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/tiancode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\tiancode`

2. Reinicie tiancode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a tiancode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
tiancode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
