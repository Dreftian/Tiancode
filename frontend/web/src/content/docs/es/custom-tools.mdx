...
description: Cree herramientas que LLM pueda llamar en tiancode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de tiancode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.tiancode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/tiancode/tools/`.
...
```ts title=".tiancode/tools/database.ts" {1}
...
```ts title=".tiancode/tools/math.ts"
import { tool } from "@tiancode-ai/plugin"
...
```ts title=".tiancode/tools/bash.ts"
import { tool } from "@tiancode-ai/plugin"
...
```ts title=".tiancode/tools/project.ts" {8}
import { tool } from "@tiancode-ai/plugin"
...
```python title=".tiancode/tools/add.py"
import sys
...
```ts title=".tiancode/tools/python-add.ts" {10}
import { tool } from "@tiancode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".tiancode/tools/add.py")
