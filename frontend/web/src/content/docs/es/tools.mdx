...
Las herramientas permiten que LLM realice acciones en su código base. Tiancode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Tiancode.
...
:::note
Esta herramienta solo está disponible cuando `TIANCODE_EXPERIMENTAL_LSP_TOOL=true` (o `TIANCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Tiancode o cuando la variable de entorno `TIANCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Tiancode:

```bash
TIANCODE_ENABLE_EXA=1 tiancode
```
