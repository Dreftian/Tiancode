---
title: Servidor
description: Interactuar con el servidor tiancode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `tiancode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente tiancode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `tiancode.local` |
...
Configure `TIANCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `tiancode`, o configure `TIANCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `tiancode serve` como a `tiancode web`.
...
Cuando ejecuta `tiancode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor tiancode para interactuar con tiancode mediante programación.
:::

Esta arquitectura permite que tiancode admita múltiples clientes y le permite interactuar con tiancode mediante programación.

Puede ejecutar `tiancode serve` para iniciar un servidor independiente. Si tienes el
tiancode TUI ejecutándose, `tiancode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Tiancode [IDE](/docs/ide).
...
El servidor tiancode expone las siguientes API.
