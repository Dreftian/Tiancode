...
Tiancode Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en Tiancode. Te suscribes a Tiancode Go y
...
Tiancode Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

Tiancode Go funciona como cualquier otro proveedor en Tiancode.

1. Inicias sesión en **<a href={console}>Tiancode Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Tiancode Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a Tiancode Go.
:::
...
Tiancode Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de Tiancode
usa el formato `tiancode-go/<model-id>`. Por ejemplo, para Kimi K3, usarías
`tiancode-go/kimi-k3` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con Tiancode.
