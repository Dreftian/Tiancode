...
Con Tiancode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Tiancode usa nuestro propio tema `tiancode`.
...
Tiancode viene con varios temas integrados.
...
- Quiere que Tiancode coincida con la apariencia de su terminal
...
Tiancode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/tiancode/themes/*.json` o `$XDG_CONFIG_HOME/tiancode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.tiancode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.tiancode/themes/*.json`
