...
Hay dos tipos de agentes en Tiancode; agentes primarios y subagentes.
...
Tiancode viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
Tiancode viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

Tiancode viene con dos agentes primarios integrados y tres subagentes integrados.
...
Un agente de solo lectura para investigar documentación externa y dependencias. Úsalo cuando necesites clonar el repositorio de una dependencia en la caché administrada de Tiancode, inspeccionar el código fuente de una librería o contrastar el código local con implementaciones upstream sin modificar tu espacio de trabajo.
...
Configure agentes en su archivo de configuración `tiancode.json`:
...
Si no se especifica ninguna temperatura, Tiancode utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global Tiancode como para la configuración específica del proyecto.
...
El ID del modelo en su configuración Tiancode usa el formato `provider/model-id`. Por ejemplo, si usa [Tiancode Zen](/docs/zen), usaría `tiancode/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `tiancode models` para ver una lista de los modelos disponibles.
:::
