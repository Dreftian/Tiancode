---
title: Proveedores
description: Usando cualquier proveedor LLM en Tiancode.
---

import config from "../../../../config.mjs"
export const console = config.console

Tiancode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Tiancode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Tiancode
configuración.
...
Tiancode Zen es una lista de modelos proporcionados por el equipo Tiancode que han sido
probado y verificado para funcionar bien con Tiancode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Tiancode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione tiancode y diríjase a [opencode.ai/auth](https://opencode.ai/auth).
...
Funciona como cualquier otro proveedor en Tiancode y su uso es completamente opcional.

---

## Tiancode Go

Tiancode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Tiancode que han sido
probado y verificado para funcionar bien con Tiancode.

1. Ejecute el comando `/connect` en TUI, seleccione `Tiancode Go` y diríjase a [opencode.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con Tiancode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta tiancode:
   ...
   Para una configuración persistente o específica del proyecto, utilice `tiancode.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Tiancode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar tiancode para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que tiancode funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración tiancode.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `Tiancode`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación tiancode.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Tiancode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Tiancode
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `tiancode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `tiancode.json`:
...
Para utilizar su suscripción GitHub Copilot con tiancode:
...
Para utilizar Google Vertex AI con Tiancode:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta tiancode.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de tiancode, siempre podrá configurarlo usted mismo.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Tiancode enumerada como una sesión separada.
...
Puede configurar tiancode para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar tiancode para usar modelos locales a través de LM Studio.
...
Puedes configurar tiancode para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Tiancode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/tiancode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Tiancode:
...
3. Copie la clave API para usarla en Tiancode.
...
6. **Importante**: Antes de usar modelos en la nube en Tiancode, debe obtener la información del modelo localmente:
...
Tiancode Zen es una lista de modelos probados y verificados proporcionada por el equipo Tiancode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Tiancode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Tiancode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Tiancode API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración tiancode.
...
5. También puedes personalizarlos a través de tu configuración tiancode. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Tiancode:
...
También puede personalizar modelos a través de su configuración tiancode. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con tiancode. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `tiancode.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Tiancode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `tiancode auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración tiancode y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración tiancode.
