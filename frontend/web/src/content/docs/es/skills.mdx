...
Las habilidades del agente permiten a Tiancode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Tiancode busca estas ubicaciones:

- Configuración del proyecto: `.tiancode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/tiancode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Tiancode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.tiancode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/tiancode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.tiancode/skills/git-release/SKILL.md` así:
...
Tiancode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `tiancode.json`:
...
**Para agentes integrados** (en `tiancode.json`):
