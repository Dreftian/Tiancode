---
title: Sunucu
description: tiancode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`tiancode serve` komutu, tiancode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `tiancode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `TIANCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `tiancode` değeridir; değiştirmek isterseniz `TIANCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `tiancode serve` hem de `tiancode web` için geçerlidir.
...
`tiancode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
tiancode ile programatik etkileşim için tiancode sunucusunu kullanın.
:::

Bu mimari, tiancode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`tiancode serve` ile bağımsız bir sunucu başlatabilirsiniz. tiancode TUI açıksa, `tiancode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum tiancode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
tiancode sunucusu aşağıdaki API'leri sunar.
