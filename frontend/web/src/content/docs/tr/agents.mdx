...
tiancode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
tiancode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Tiancode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Tiancode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `tiancode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse tiancode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global tiancode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
tiancode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Tiancode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `tiancode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `tiancode models` komutunu çalıştırın.
:::
