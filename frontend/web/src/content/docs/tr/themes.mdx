...
tiancode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak tiancode kendi `tiancode` temasını kullanır.
...
tiancode birden fazla yerleşik temayla gelir.
...
- tiancode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
tiancode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/tiancode/themes/*.json` veya `$XDG_CONFIG_HOME/tiancode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.tiancode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.tiancode/themes/*.json`
