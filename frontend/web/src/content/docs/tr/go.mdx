...
Tiancode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Tiancode'daki diğer sağlayıcılar gibi çalışır. Tiancode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Tiancode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Tiancode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Tiancode Go, Tiancode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Tiancode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Tiancode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Tiancode Go'ya abone olabilir.
:::
...
Tiancode Go aşağıdaki limitleri içerir:
...
Tiancode yapılandırmanızdaki [model id](/docs/config/#models) formatı `tiancode-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `tiancode-go/kimi-k3` kullanmalısınız.
...
Tiancode Go'yu şu amaçlarla oluşturduk:
...
4. Tiancode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
