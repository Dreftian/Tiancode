...
_Model Bağlam Protokolü_ veya MCP kullanarak tiancode'a harici araçlar ekleyebilirsiniz. tiancode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [tiancode Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/tiancode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
tiancode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde tiancode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa tiancode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `tiancode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından tiancode, belirteçleri `~/.local/share/tiancode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra tiancode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi tiancode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve tiancode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
