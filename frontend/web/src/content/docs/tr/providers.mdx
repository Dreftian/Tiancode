---
title: Sağlayıcılar
description: tiancode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

tiancode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı tiancode yapılandırmanızda yapılandırın.
...
`~/.local/share/tiancode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları tiancode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Tiancode Zen, tiancode ekibi tarafından sağlanan modellerin bir listesidir.
tiancode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Tiancode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, tiancode'u seçin ve [opencode.ai/auth](https://opencode.ai/auth)'ye gidin.
...
tiancode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Tiancode Go

Tiancode Go, tiancode ile iyi çalıştığı test edilmiş ve doğrulanmış, tiancode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Tiancode Go`'yu seçin ve [opencode.ai/auth](https://opencode.ai/zen) adresine gidin.
...
Amazon Bedrock'u tiancode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   tiancode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `tiancode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi tiancode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
tiancode'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   tiancode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
tiancode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. tiancode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`tiancode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`tiancode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi tiancode'la kullanmak için:
...
Google Vertex AI'yi tiancode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları tiancode çalıştırılırken ayarlayın.
...
Helicone'un tiancode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her tiancode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [tiancode-helicone-session](https://github.com/H2Shami/tiancode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her tiancode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
tiancode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla tiancode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla tiancode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini tiancode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/tiancode)'a bakın.
:::
...
Ollama Cloud'u tiancode ile kullanmak için:
...
3. tiancode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: tiancode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Tiancode Zen, tiancode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Tiancode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Tiancode Zen**'i arayın.

   ```txt
   /connect
   ```

3. tiancode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   tiancode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca tiancode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri tiancode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı tiancode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `tiancode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları tiancode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `tiancode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için tiancode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, tiancode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
