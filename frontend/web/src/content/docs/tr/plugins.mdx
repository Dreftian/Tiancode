---
title: Eklentiler
description: Tiancode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Tiancode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Tiancode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.tiancode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/tiancode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/tiancode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/tiancode/tiancode.json`)
2. Project config (`tiancode.json`)
3. Global eklenti dizini (`~/.config/tiancode/plugins/`)
4. Proje eklenti dizini (`.tiancode/plugins/`)
...
Tiancode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Tiancode SDK istemcisi.
...
Tiancode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Tiancode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Tiancode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Tiancode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Tiancode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Tiancode'a da hazır olacaktır.
