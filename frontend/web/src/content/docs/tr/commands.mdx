...
`.tiancode/commands/test.md` oluştur:
...
tiancode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

tiancode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/tiancode/commands/`
- Proje başına: `.tiancode/commands/`
...
tiancode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
