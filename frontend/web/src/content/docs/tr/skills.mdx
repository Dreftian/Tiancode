...
Ajan becerileri, tiancode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
tiancode su konumlari tarar:

- Proje konfigurasyonu: `.tiancode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/tiancode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda tiancode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.tiancode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/tiancode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.tiancode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
tiancode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`tiancode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`tiancode.json` icinde):
