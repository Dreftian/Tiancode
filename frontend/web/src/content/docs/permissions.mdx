...
Tiancode uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start Tiancode with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
tiancode --auto
```

You can also use auto mode with [`tiancode run`](/docs/cli#run).

```bash
tiancode run --auto "Refactor this module"
```
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Tiancode was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
Tiancode permissions are keyed by tool name, plus a couple of safety guards:
...
If you don’t specify anything, Tiancode starts from permissive defaults:
...
When Tiancode prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current Tiancode session)
