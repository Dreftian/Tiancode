---
title: Server
description: Interact with tiancode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `tiancode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an tiancode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `tiancode.local` |
...
Set `TIANCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `tiancode`, or set `TIANCODE_SERVER_USERNAME` to override it. This applies to both `tiancode serve` and `tiancode web`.
...
When you run `tiancode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the tiancode server to interact with tiancode programmatically.
:::

This architecture lets tiancode support multiple clients and allows you to interact with tiancode programmatically.

You can run `tiancode serve` to start a standalone server. If you have the
tiancode TUI running, `tiancode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Tiancode [IDE](/docs/ide) plugins.
...
The tiancode server exposes the following APIs.
