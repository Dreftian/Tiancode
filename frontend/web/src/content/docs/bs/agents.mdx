...
Postoje dvije vrste agenata u Tiancode: primarni agenti i podagenti.
...
Tiancode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Tiancode dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Tiancode dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Tiancode-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `tiancode.json`:
...
- Globalno: `~/.config/tiancode/agents/`
- Po projektu: `.tiancode/agents/`
...
Ako temperatura nije navedena, Tiancode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Tiancode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Tiancode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Tiancode Zen](/docs/zen), koristili biste `tiancode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `tiancode models` da vidite listu dostupnih modela.
:::
