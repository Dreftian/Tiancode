...
U Tiancode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Tiancode koristi nasu `tiancode` temu.
...
Tiancode dolazi sa vise ugradenih tema.
...
- Zele da Tiancode odgovara izgledu njihovog terminala
...
Tiancode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/tiancode/themes/*.json` ili `$XDG_CONFIG_HOME/tiancode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.tiancode/themes/*.json`
4. **Trenutni radni direktorij** - `./.tiancode/themes/*.json`
