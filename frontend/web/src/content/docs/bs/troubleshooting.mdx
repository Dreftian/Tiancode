...
Da biste otklonili probleme s Tiancode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\tiancode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `tiancode --log-level DEBUG`.

---

## Pohrana

Tiancode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/tiancode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\tiancode`
...
Tiancode Desktop pokreće lokalni Tiancode server (`tiancode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Tiancode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/tiancode/tiancode.jsonc` (ili `~/.config/tiancode/tiancode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/tiancode/tiancode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\tiancode\tiancode.jsonc`
...
Tiancode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/tiancode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\tiancode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Tiancode mogao ponovo izgraditi.

1. Potpuno zatvorite Tiancode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/tiancode`
- **Linux**: obrišite `~/.cache/tiancode` (ili pokrenite `rm -rf ~/.cache/tiancode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\tiancode`

3. Ponovo pokrenite Tiancode Desktop.

---

### Rješavanje problema sa vezom na serveru

Tiancode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `tiancode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `TIANCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `TIANCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Tiancode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Tiancode karakteristikama.

---

### Obavještenja se ne prikazuju

Tiancode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Tiancode u postavkama vašeg OS-a, i
...
1. Zatvorite Tiancode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Tiancode Desktop aplikacije):

- `tiancode.settings.dat` (URL zadanog servera za desktop)
- `tiancode.global.dat` i `tiancode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `tiancode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\tiancode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Tiancode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\tiancode`

2. Ponovo pokrenite Tiancode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Tiancode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Tiancode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
