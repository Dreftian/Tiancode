...
Možete dodati vanjske alate u Tiancode koristeći _Model Context Protocol_, ili MCP. Tiancode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Tiancode Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/tiancode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Tiancode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Tiancode će:
...
Ako server zahtijeva autentifikaciju, Tiancode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `tiancode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Tiancode sigurno čuva tokene u `~/.local/share/tiancode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Tiancode, zajedno s ugrađenim alatima. Možete njima upravljati kroz Tiancode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Tiancode sa vašim Sentry nalogom.
