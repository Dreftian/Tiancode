---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Tiancode.
---

import config from "../../../../config.mjs"
export const console = config.console

Tiancode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Tiancode konfiguraciji.
...
u `~/.local/share/tiancode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Tiancode
config.
...
Tiancode Zen je lista modela koje je obezbedio Tiancode tim koji su bili
testirano i potvrđeno da dobro radi sa Tiancode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Tiancode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite tiancode i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u Tiancode i potpuno je opcionalan za korištenje.

---

## Tiancode Go

Tiancode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Tiancode tim i koji su testirani i verificirani da dobro rade s Tiancode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Tiancode Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s Tiancode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete tiancode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `tiancode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Tiancode.
:::
...
Možete konfigurirati tiancode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi tiancode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju tiancode konfiguraciju.
...
da pohrani token u tiancode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Tiancode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Tiancode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`tiancode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `tiancode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s tiancode:
...
Za korištenje Google Vertex AI s Tiancode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće tiancode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem tiancodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [tiancode-helicone-session](https://github.com/H2Shami/tiancode-helicone-session) da automatski prijavite svaki Tiancode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Tiancode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati tiancode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati tiancode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati tiancode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Tiancode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/tiancode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Tiancode:
...
3. Kopirajte API ključ za korištenje u Tiancode.
...
6. **Važno**: Prije upotrebe modela oblaka u Tiancode, morate lokalno povući informacije o modelu:
...
Tiancode Zen je lista testiranih i verifikovanih modela koju je obezbedio Tiancode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Tiancode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Tiancode Zen**.

```txt
   /connect
```

3. Unesite svoj Tiancode API ključ.
...
Također možete dodati dodatne modele putem vaše tiancode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše tiancode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju tiancode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s tiancode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `tiancode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Tiancode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `tiancode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Tiancode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj tiancode konfiguraciji.
