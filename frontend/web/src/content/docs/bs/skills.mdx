...
Agent skills omogucavaju Tiancode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Tiancode pretrazuje ove lokacije:

- Konfiguracija projekta: `.tiancode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/tiancode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Tiancode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.tiancode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/tiancode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.tiancode/skills/git-release/SKILL.md` ovako:
...
Tiancode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `tiancode.json`:
...
**Za ugradene agente** (u `tiancode.json`):
