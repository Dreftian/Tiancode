---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Tiancode.
---

Dodaci vam omogućavaju da proširite Tiancode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Tiancode.
...
- `.tiancode/plugins/` - Dodaci na nivou projekta
- `~/.config/tiancode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/tiancode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/tiancode/tiancode.json`)
2. Konfiguracija projekta (`tiancode.json`)
3. Globalni direktorij dodataka (`~/.config/tiancode/plugins/`)
4. Direktorij dodataka projekta (`.tiancode/plugins/`)
...
Tiancode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Tiancode.
...
Spriječite tiancode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u tiancode:
...
Pomoćnik `tool` kreira prilagođeni alat koji tiancode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za tiancode zajedno sa ugrađenim alatima.
