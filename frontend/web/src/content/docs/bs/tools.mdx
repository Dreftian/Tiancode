...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Tiancode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Tiancode.
...
:::note
Ovaj alat je dostupan samo kada je `TIANCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `TIANCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Tiancode provajdera ili kada je varijabla `TIANCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Tiancode:
