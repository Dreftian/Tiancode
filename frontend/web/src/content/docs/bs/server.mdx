---
title: Server
description: Komunicirajte s tiancode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `tiancode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji tiancode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `tiancode.local` |
...
Postavite `TIANCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `tiancode`, ili postavite `TIANCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `tiancode serve` i za `tiancode web`.
...
Kada pokrenete `tiancode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite tiancode server za programsku interakciju sa tiancode-om.
:::

Ova arhitektura omogucava tiancode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `tiancode serve` da startate standalone server. Ako je tiancode TUI vec pokrenut, `tiancode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Tiancode [IDE](/docs/ide) pluginovi.
...
tiancode server izlaže sljedece API-je.
