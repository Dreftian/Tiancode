---
title: LSP serveri
description: Tiancode se integriše sa vašim LSP serverima.
---

Tiancode se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

Tiancode dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `TIANCODE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i tiancode otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj tiancode konfiguraciji.
