...
Wenn Tiancode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\tiancode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `tiancode --log-level DEBUG`.

---

## Speicher

tiancode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/tiancode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\tiancode`
...
Tiancode Desktop startet im Hintergrund einen lokalen Tiancode-Server (`tiancode-cli`-Sidecar).
...
- Nur macOS: `Tiancode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/tiancode/tiancode.jsonc` (oder `~/.config/tiancode/tiancode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/tiancode/tiancode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\tiancode\tiancode.jsonc`
...
Tiancode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/tiancode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\tiancode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Tiancode ihn neu aufbauen kann.

1. Beende Tiancode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/tiancode`
- **Linux**: loesche `~/.cache/tiancode` (oder fuehre aus: `rm -rf ~/.cache/tiancode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\tiancode`

3. Starte Tiancode Desktop neu.

---

### Server-Verbindungsprobleme beheben

Tiancode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `tiancode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `TIANCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `TIANCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Tiancode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Tiancode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Tiancode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Tiancode in den OS-Einstellungen aktiviert sind, und
...
1. Beende Tiancode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Tiancode Desktop):

- `tiancode.settings.dat` (desktop default server URL)
- `tiancode.global.dat` und `tiancode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Tiancode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `tiancode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\tiancode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. tiancode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\tiancode`

2. Starte tiancode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt tiancode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
tiancode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
