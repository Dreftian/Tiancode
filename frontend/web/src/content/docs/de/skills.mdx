...
Agent Skills erlauben Tiancode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Tiancode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Tiancode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.tiancode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/tiancode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.tiancode/skills/git-release/SKILL.md` zum Beispiel so:
...
Tiancode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `tiancode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `tiancode.json`):
