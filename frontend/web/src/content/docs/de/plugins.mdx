---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Tiancode zu erweitern.
---

Mit Plugins können Sie Tiancode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Tiancode zu ändern.
...
- `.tiancode/plugins/` – Plugins auf Projektebene
- `~/.config/tiancode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/tiancode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/tiancode/tiancode.json`)
2. Projektkonfiguration (`tiancode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/tiancode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.tiancode/plugins/`)
...
Tiancode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Tiancode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Tiancode erweitern können.
...
:::note
Wenn Sie die Tiancode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Tiancode `.env`-Dateien liest:
...
Plugins können Tiancode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Tiancode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Tiancode zur Verfügung.
