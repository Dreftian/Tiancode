---
title: Regeln
description: Lege eigene Anweisungen fuer tiancode fest.
---

Du kannst tiancode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in tiancode den Befehl `/init` aus.
...
So kann tiancode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht tiancode sie zu erweitern.
...
tiancode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/tiancode/AGENTS.md` speichern.
Diese gelten in allen tiancode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Tiancode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/tiancode/AGENTS.md` existiert)
...
Beim Start sucht tiancode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/tiancode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `tiancode.json` oder in `~/.config/tiancode/tiancode.json` angeben.
...
tiancode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit tiancode.json

Empfohlen ist das `instructions`-Feld in der `tiancode.json`:
...
Du kannst tiancode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `tiancode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
