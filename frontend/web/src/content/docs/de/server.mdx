---
title: Server
description: Nutze den tiancode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `tiancode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein tiancode-Client nutzen kann.
...
Setze `TIANCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `tiancode`, kann aber mit `TIANCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `tiancode serve` und `tiancode web`.
...
Wenn du `tiancode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den tiancode-Server, um tiancode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `tiancode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `tiancode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Tiancode-[IDE](/docs/ide)-Plugins.
...
Der tiancode-Server stellt folgende APIs bereit.
