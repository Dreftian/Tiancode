...
Mit Tiancode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Tiancode das `tiancode`-Theme.
...
Tiancode liefert mehrere eingebaute Themes mit.
...
- Tiancode optisch ans Terminal anpassen wollen
...
Tiancode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/tiancode/themes/*.json` oder `$XDG_CONFIG_HOME/tiancode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.tiancode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.tiancode/themes/*.json`
