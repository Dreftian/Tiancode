...
Es gibt zwei Arten von Agenten in Tiancode; Primäragenten und Subagenten.
...
Tiancode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `tiancode.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Tiancode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Tiancode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Tiancode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Tiancode Zen](/docs/zen) verwenden, würden Sie `tiancode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `tiancode models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
