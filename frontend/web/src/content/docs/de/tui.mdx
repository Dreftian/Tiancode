---
title: TUI
description: Arbeite mit Tiancode in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Tiancode bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet Tiancode die TUI im aktuellen Verzeichnis.
...
In der Tiancode-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Fuegt einen Provider zu Tiancode hinzu. Erlaubt die Auswahl aus verfuegbaren Providern und das Hinzufuegen ihrer API-Keys.
...
Beendet Tiancode. _Aliase_: `/quit`, `/q`
...
Dies ist getrennt von `tiancode.json`, welche das Server-/Runtime-Verhalten konfiguriert.
...
- `leader_timeout` - Steuert, wie lange Tiancode nach der leader key wartet. Standard ist `2000`.
...
- `sound_pack` - Zu verwendende sound pack ID. Standard ist `tiancode.default`.
