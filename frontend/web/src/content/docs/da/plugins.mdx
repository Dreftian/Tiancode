---
title: Plugins
description: Skriv dine egne plugins for at udvide Tiancode.
---

Plugins giver dig mulighed for at udvide Tiancode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Tiancodes standardadfærd.
...
- `.tiancode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/tiancode/node_modules/`.
...
1. Global konfiguration (`~/.config/tiancode/tiancode.json`)
2. Projektkonfiguration (`tiancode.json`)
3. Global plugin-mappe (`~/.config/tiancode/plugins/`)
4. Projekt plugin bibliotek (`.tiancode/plugins/`)
...
Tiancode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En tiancode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide tiancode.
...
:::note
Hvis du bruger Tiancode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå tiancode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til tiancode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som tiancode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for tiancode sammen med indbyggede værktøjer.
