---
title: Konfiguration
description: Ved at bruge Tiancode JSON konfig.
---

Du kan konfigurere Tiancode ved hjælp af en JSON-konfigurationsfil.

---

## Format

Tiancode understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/tiancode`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/tiancode/tiancode.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`TIANCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`tiancode.json` i projekt) - projektspecifikke indstillinger
5. **`.tiancode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`TIANCODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.tiancode` og `~/.config/tiancode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/tiancode`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Tiancode-konfiguration i `~/.config/tiancode/tiancode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `tiancode.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Tiancode starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `TIANCODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `TIANCODE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.tiancode` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.tiancode` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan konfigurere serverindstillinger for kommandoerne `tiancode serve` og `tiancode web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Tiancode-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `tiancode.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Tiancode-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Tiancode at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Tiancode-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/tiancode/agents/` eller `.tiancode/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Tiancode falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`tiancode run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/tiancode/commands/` eller `.tiancode/commands/`. [Learn more here](/docs/commands).
...
Tiancode vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard tiancode **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Tiancode med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.tiancode/plugins/` eller `~/.config/tiancode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Tiancode til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
