...
Agentferdigheter lar Tiancode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Tiancode søker etter disse stedene:

- Prosjektkonfigurasjon: `.tiancode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/tiancode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Tiancode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.tiancode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/tiancode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.tiancode/skills/git-release/SKILL.md` slik:
...
Tiancode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `tiancode.json`:
...
**For innebygde agenter** (i `tiancode.json`):
