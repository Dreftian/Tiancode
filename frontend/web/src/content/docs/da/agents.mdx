...
Der er to typer agenter i Tiancode; primære agenter og subagenter.
...
Tiancode leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Tiancode leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

Tiancode leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i Tiancode's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `tiancode.json`-konfigurationsfil:
...
- Globalt: `~/.config/tiancode/agents/`
- Pr. projekt: `.tiancode/agents/`
...
Hvis ingen temperatur er angivet, bruger Tiancode modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Tiancode-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Tiancode-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Tiancode Zen](/docs/zen), vil du bruge `tiancode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `tiancode models` for at se en liste over de tilgængelige modeller.
:::
