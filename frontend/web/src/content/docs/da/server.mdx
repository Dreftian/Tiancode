---
title: Server
description: Interager med tiancode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `tiancode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en tiancode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `tiancode.local` |
...
Indstil `TIANCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `tiancode`, eller indstil `TIANCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `tiancode serve` og `tiancode web`.
...
Når du kører `tiancode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug tiancode-serveren til at interagere med tiancode programmatisk.
:::

Denne arkitektur lader tiancode understøtte flere klienter og lader dig interagere med tiancode programmatisk.

Du kan køre `tiancode serve` for at starte en selvstændig server. Hvis du har
tiancode TUI kørende, vil `tiancode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Tiancode [IDE](/docs/ide) plugins.
...
Tiancode-serveren viser følgende API'er.
