...
Her er alle de innebygde verktøyene som er tilgængelige i Tiancode.
...
:::note
Dette verktøyet er bare tilgængelig når `TIANCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `TIANCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Tiancode-leverandøren eller når miljøvariabelen `TIANCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
