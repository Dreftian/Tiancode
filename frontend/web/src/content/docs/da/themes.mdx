...
Med Tiancode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Tiancode vårt eget `tiancode`-tema.
...
Tiancode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/tiancode/themes/*.json` eller `$XDG_CONFIG_HOME/tiancode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.tiancode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.tiancode/themes/*.json`
