...
For at fejlfinde problemer med Tiancode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\tiancode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `tiancode --log-level DEBUG`.

---

## Lagring

tiancode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/tiancode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\tiancode`
...
Tiancode Desktop kører en lokal Tiancode-server (`tiancode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Tiancode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/tiancode/tiancode.jsonc` (eller `~/.config/tiancode/tiancode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/tiancode/tiancode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\tiancode\tiancode.jsonc`
...
Tiancode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/tiancode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\tiancode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Tiancode kan genopbygge den.

1. Afslut Tiancode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/tiancode`
- **Linux**: slet `~/.cache/tiancode` (eller kør `rm -rf ~/.cache/tiancode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\tiancode`

3. Start Tiancode Desktop på ny.

---

### Løs problemer med serverforbindelse

Tiancode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `tiancode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `TIANCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `TIANCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Tiancode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Tiancode.

---

### Meddelelser vises ikke

Tiancode Desktop viser kun systemvarsler når:

- varsler er aktiveret for Tiancode i dine OS-indstillinger, og
...
1. Afslut Tiancode Desktop.
2. Find og slet disse filer (de findes i Tiancode Desktop-appens datamappe):

- `tiancode.settings.dat` (skrivebordsstandardserver URL)
- `tiancode.global.dat` og `tiancode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Tiancode:
...
For at finde ud af hvilke modeller du har adgang til, kør `tiancode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\tiancode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. tiancode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\tiancode`

2. Start tiancode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge tiancode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
tiancode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
