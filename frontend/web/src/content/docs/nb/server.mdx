---
title: Tjener
description: Samhandle med Tiancode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `tiancode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Tiancode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `tiancode.local` |
...
Sett `TIANCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `tiancode` som standard, eller sett `TIANCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `tiancode serve` og `tiancode web`.
...
Når du kjører `tiancode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Tiancode-serveren til å samhandle med Tiancode programmatisk.
:::

Denne arkitekturen lar Tiancode støtte flere klienter og lar deg samhandle med Tiancode programmatisk.

Du kan kjøre `tiancode serve` for å starte en frittstående server. Hvis du har
Tiancode TUI kjører, vil `tiancode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Tiancode [IDE](/docs/ide) plugins.
...
tiancode-serveren viser følgende APIer.
