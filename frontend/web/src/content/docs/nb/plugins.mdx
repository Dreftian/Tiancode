...
description: Skriv dine egne programtillegg for å utvide Tiancode.
---

Plugins lar deg utvide Tiancode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Tiancode.
...
- `.tiancode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/tiancode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/tiancode/tiancode.json`)
2. Prosjektkonfigurasjon (`tiancode.json`)
3. Global plugin-katalog (`~/.config/tiancode/plugins/`)
4. Prosjektpluginkatalog (`.tiancode/plugins/`)
...
Tiancode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Tiancode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Tiancode.
...
:::note
Hvis du bruker Tiancode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Tiancode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Tiancode:
...
`tool`-hjelperen lager et tilpasset verktøy som Tiancode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Tiancode sammen med innebygde verktøy.
