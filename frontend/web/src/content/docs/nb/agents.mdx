...
Det er to typer agenter i Tiancode; primære agenter og underagenter.
...
Tiancode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Tiancode kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Tiancode kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `tiancode.json`:
...
Hvis ingen temperatur er spesifisert, bruker Tiancode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Tiancode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Tiancode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Tiancode Zen](/docs/zen), vil du bruke `tiancode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `tiancode models` for å se en liste over tilgjengelige modeller.
:::
