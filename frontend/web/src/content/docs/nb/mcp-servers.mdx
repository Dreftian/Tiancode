...
Du kan legge til eksterne verktøy i Tiancode ved å bruke _Model Context Protocol_ eller MCP. Tiancode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Tiancode Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="tiancode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/tiancode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Tiancode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Tiancode:
...
Hvis serveren krever autentisering, vil Tiancode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `tiancode mcp auth <server-name>`.
...
```bash
tiancode mcp auth my-oauth-server
```
...
```bash
tiancode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Tiancode lagre tokens sikkert i `~/.local/share/tiancode/mcp-auth.json`.
...
tiancode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Tiancode, sammen med innebygde verktøy. Så du kan administrere dem gjennom Tiancode-konfigurasjonen som alle andre verktøy.
...
```json title="tiancode.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
tiancode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Tiancode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Tiancode-samtale oppført som en separat økt.
