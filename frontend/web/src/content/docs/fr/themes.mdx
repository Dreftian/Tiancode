...
Avec Tiancode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Tiancode utilise notre propre thème `tiancode`.
...
Tiancode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Tiancode corresponde à l'apparence de leur terminal
...
Tiancode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/tiancode/themes/*.json` ou `$XDG_CONFIG_HOME/tiancode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.tiancode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.tiancode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/tiancode/themes
vim ~/.config/tiancode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .tiancode/themes
vim .tiancode/themes/my-theme.json
```
