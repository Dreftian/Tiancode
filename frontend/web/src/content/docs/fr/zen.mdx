---
title: Zen
description: Liste organisée de modèles fournis par Tiancode.
---
...
Tiancode Zen est une liste de modèles testés et vérifiés fournie par l'équipe Tiancode.

Zen fonctionne comme n'importe quel autre fournisseur dans Tiancode. Vous vous connectez à Tiancode Zen et obtenez votre clé API. C'est **entièrement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Tiancode.
...
:::tip
Nous avons testé un groupe sélectionné de modèles et de fournisseurs qui fonctionnent bien avec Tiancode.
:::
...
Tiancode Zen est une passerelle AI qui vous donne accès à ces modèles.

---

## Comment ça marche

Tiancode Zen fonctionne comme n'importe quel autre fournisseur dans Tiancode.

1. Vous vous connectez à **<a href={console}>Tiancode Zen</a>**, ajoutez vos informations de facturation et copiez votre clé API.
2. Vous exécutez la commande `/connect` dans le TUI, sélectionnez Tiancode Zen et collez votre clé API.
...
Le [model id](/docs/config/#models) dans votre configuration Tiancode utilise le format `tiancode/<model-id>`. Par exemple, pour GPT 5.5, vous utiliseriez `tiancode/gpt-5.5` dans votre configuration.
...
Vous remarquerez peut-être des [modèles à faible coût](/docs/config/#models), tels que Haiku, Nano ou Flash, dans votre historique d'utilisation. Tiancode utilise ces modèles pour générer les titres des sessions.
...
- DeepSeek V4 Flash Free est disponible sur Tiancode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- MiMo-V2.5 Free est disponible sur Tiancode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Laguna S 2.1 Free est disponible sur Tiancode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Ling-3.0-flash Free est disponible sur Tiancode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- LongCat-2.0 Free est disponible sur Tiancode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- North Mini Code Free est disponible sur Tiancode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3 Ultra Free est disponible sur Tiancode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Big Pickle est un modèle stealth gratuit sur Tiancode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
...
Nous avons créé Tiancode Zen pour :
...
4. N'avoir **aucun lock-in** en vous permettant de l'utiliser avec n'importe quel autre agent de codage. Et toujours vous permettre d'utiliser n'importe quel autre fournisseur avec Tiancode également.
