...
Il existe deux types d'agents dans Tiancode ; agents primaires et sous-agents.
...
Tiancode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Tiancode est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

Tiancode est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `tiancode.json` :
...
Si aucune température n'est spécifiée, Tiancode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Tiancode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Tiancode utilise le format `provider/model-id`. Par exemple, si vous utilisez [Tiancode Zen](/docs/zen), vous utiliserez `tiancode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `tiancode models` pour voir une liste des modèles disponibles.
:::
