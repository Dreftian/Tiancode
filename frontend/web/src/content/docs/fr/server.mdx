---
title: Serveur
description: Interagissez avec le serveur tiancode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `tiancode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client tiancode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `tiancode.local` |
...
Définissez `TIANCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `tiancode`, ou définissez `TIANCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `tiancode serve` et à `tiancode web`.
...
Lorsque vous exécutez `tiancode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur tiancode pour interagir avec tiancode par programmation.
:::

Cette architecture permet à tiancode de prendre en charge plusieurs clients et vous permet d'interagir avec tiancode par programme.

Vous pouvez exécuter `tiancode serve` pour démarrer un serveur autonome. Si vous avez le
tiancode TUI en cours d'exécution, `tiancode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Tiancode [IDE](/docs/ide).
...
Le serveur tiancode expose les API suivantes.
