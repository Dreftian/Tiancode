---
title: Plugins
description: Écrivez vos propres plugins pour étendre Tiancode.
---

Les plugins vous permettent d'étendre Tiancode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Tiancode.
...
- `.tiancode/plugins/` - Plugins au niveau du projet
- `~/.config/tiancode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/tiancode/node_modules/`.
...
1. Configuration globale (`~/.config/tiancode/tiancode.json`)
2. Configuration du projet (`tiancode.json`)
3. Répertoire global des plugins (`~/.config/tiancode/plugins/`)
4. Répertoire des plugins du projet (`.tiancode/plugins/`)
...
Tiancode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK tiancode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre tiancode.
...
:::note
Si vous utilisez l'application de bureau Tiancode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez tiancode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à tiancode :
...
L'assistant `tool` crée un outil personnalisé que tiancode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour tiancode aux côtés des outils intégrés.
