---
title: TUI
description: Using the Tiancode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Tiancode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Tiancode starts the TUI for the current directory.
...
When using the Tiancode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Tiancode. Allows you to select from available providers and add their API keys.
...
Exit Tiancode. _Aliases_: `/quit`, `/q`
...
This is separate from `tiancode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Tiancode waits after the leader key. Defaults to `2000`.
...
Use `TIANCODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `tiancode.default`.
