...
You can add external tools to Tiancode using the _Model Context Protocol_, or MCP. Tiancode supports both local and remote servers.
...
You can define MCP servers in your [Tiancode Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/tiancode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Tiancode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Tiancode will:
...
If the server requires authentication, Tiancode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `tiancode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Tiancode will store the tokens securely in `~/.local/share/tiancode/mcp-auth.json`.
...
Your MCPs are available as tools in Tiancode, alongside built-in tools. So you can manage them through the Tiancode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Tiancode to your Sentry account.
