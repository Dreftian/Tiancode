---
title: Config
description: Using the Tiancode JSON config.
---

You can configure Tiancode using a JSON config file.

---

## Format

Tiancode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="tiancode.jsonc"
{
...
1. **Remote config** (from `.well-known/tiancode`) - organizational defaults
2. **Global config** (`~/.config/tiancode/tiancode.json`) - user preferences
3. **Custom config** (`TIANCODE_CONFIG` env var) - custom overrides
4. **Project config** (`tiancode.json` in project) - project-specific settings
5. **`.tiancode` directories** - agents, commands, plugins
6. **Inline config** (`TIANCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/tiancode/` on macOS) - admin-controlled
...
:::note
The `.tiancode` and `~/.config/tiancode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/tiancode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/tiancode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Tiancode config in `~/.config/tiancode/tiancode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/tiancode/tui.json`.
...
Add `tiancode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Tiancode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `TIANCODE_CONFIG` environment variable.

```bash
export TIANCODE_CONFIG=/path/to/my/custom-config.json
tiancode run "Hello world"
```
...
Specify a custom config directory using the `TIANCODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.tiancode` directory, and should
follow the same structure.

```bash
export TIANCODE_CONFIG_DIR=/path/to/my/config-directory
tiancode run "Hello world"
```

The custom directory is loaded after the global config and `.tiancode` directories, so it **can override** their settings.
...
Drop an `tiancode.json` or `tiancode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/tiancode/` |
| Linux    | `/etc/tiancode/`                         |
| Windows  | `%ProgramData%\tiancode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Tiancode reads managed preferences from the `ai.tiancode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Tiancode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.tiancode.managed.plist`
2. `/Library/Managed Preferences/ai.tiancode.managed.plist`

The plist keys map directly to `tiancode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.tiancode.managed` PayloadType. The Tiancode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.tiancode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.tiancode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.tiancode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Use `TIANCODE_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `tiancode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `tiancode serve` and `tiancode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Tiancode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `tiancode.local`. Useful for running multiple instances on the same network.
...
If not specified, Tiancode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Tiancode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Tiancode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Tiancode actions on configured resources. Currently, policies can control which providers Tiancode may use.
...
Tiancode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Tiancode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/tiancode/agents/` or `.tiancode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Tiancode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`tiancode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/tiancode/commands/` or `.tiancode/commands/`. [Learn more here](/docs/commands).
...
Tiancode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Tiancode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, tiancode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Tiancode with custom tools, hooks, and integrations.

Place plugin files in `.tiancode/plugins/` or `~/.config/tiancode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="tiancode.json"
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["tiancode-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Tiancode to only use specific providers rather than disabling them one by one.
