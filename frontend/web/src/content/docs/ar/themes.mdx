...
مع Tiancode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Tiancode سمتنا `tiancode`.
...
يأتي Tiancode مع عدة سمات مدمجة.
...
- يريدون أن يطابق Tiancode مظهر terminal لديهم
...
يدعم Tiancode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/tiancode/themes/*.json` أو `$XDG_CONFIG_HOME/tiancode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.tiancode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.tiancode/themes/*.json`
