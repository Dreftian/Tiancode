...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في tiancode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في tiancode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.tiancode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/tiancode/tools/`.
