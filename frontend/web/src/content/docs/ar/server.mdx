---
title: الخادم
description: تفاعل مع خادم tiancode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `tiancode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل tiancode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `tiancode.local` |
...
عيّن `TIANCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `tiancode`، أو عيّن `TIANCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `tiancode serve` و `tiancode web`.
...
عند تشغيل `tiancode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم tiancode للتفاعل مع tiancode برمجيا.
:::

تتيح هذه البنية لـ tiancode دعم عدة عملاء وتمكّنك من التفاعل مع tiancode برمجيا.

يمكنك تشغيل `tiancode serve` لبدء خادم مستقل. إذا كانت واجهة tiancode في terminal (TUI)
قيد التشغيل، فسيبدأ `tiancode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Tiancode لـ [IDE](/docs/ide).
...
يعرض خادم tiancode واجهات API التالية.
