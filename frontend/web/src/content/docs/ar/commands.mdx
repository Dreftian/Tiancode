...
أنشئ الملف `.tiancode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Tiancode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Tiancode:
...
- على مستوى النظام: `~/.config/tiancode/commands/`
- لكل مشروع: `.tiancode/commands/`
...
يتضمن tiancode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
