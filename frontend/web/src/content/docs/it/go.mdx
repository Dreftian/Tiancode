...
Tiancode Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Tiancode. Ti abboni a Tiancode Go e
...
Tiancode Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Tiancode Go funziona come qualsiasi altro provider in Tiancode.

1. Accedi a **<a href={console}>Tiancode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Tiancode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Tiancode Go.
:::
...
Tiancode Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Tiancode config
utilizza il formato `tiancode-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`tiancode-go/kimi-k3` nella tua configurazione.
...
Abbiamo creato Tiancode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Tiancode.
