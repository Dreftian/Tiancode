---
title: Plugin
description: Scrivi plugin per estendere Tiancode.
---

I plugin ti permettono di estendere Tiancode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Tiancode.
...
- `.tiancode/plugins/` - plugin a livello progetto
- `~/.config/tiancode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/tiancode/node_modules/`.
...
1. Config globale (`~/.config/tiancode/tiancode.json`)
2. Config di progetto (`tiancode.json`)
3. Directory plugin globale (`~/.config/tiancode/plugins/`)
4. Directory plugin di progetto (`.tiancode/plugins/`)
...
Tiancode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di tiancode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere tiancode.
...
:::note
Se usi l'app desktop di Tiancode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a tiancode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a tiancode:
...
L'helper `tool` crea uno strumento personalizzato che tiancode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in tiancode insieme agli strumenti integrati.
