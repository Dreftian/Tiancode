---
title: Server
description: Interagisci con il server tiancode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `tiancode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client tiancode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `tiancode.local` |
...
Imposta `TIANCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `tiancode`, oppure imposta `TIANCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `tiancode serve` sia per `tiancode web`.
...
Quando esegui `tiancode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server tiancode per interagire con tiancode in modo programmatico.
:::

Questa architettura permette a tiancode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `tiancode serve` per avviare un server standalone. Se la TUI di tiancode e gia in esecuzione, `tiancode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Tiancode.
...
Il server tiancode espone le seguenti API.
