...
Con Tiancode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Tiancode usa il tema `tiancode`.
...
Tiancode include diversi temi integrati.
...
- Vuole che Tiancode corrisponda all'aspetto del terminale
...
Tiancode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/tiancode/themes/*.json` o `$XDG_CONFIG_HOME/tiancode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.tiancode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.tiancode/themes/*.json`
