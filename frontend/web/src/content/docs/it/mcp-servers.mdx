...
Puoi aggiungere strumenti esterni a Tiancode usando il _Model Context Protocol_, o MCP. Tiancode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Tiancode Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/tiancode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Tiancode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Tiancode:
...
Se il server richiede autenticazione, Tiancode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `tiancode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Tiancode memorizzera' i token in modo sicuro in `~/.local/share/tiancode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Tiancode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Tiancode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Tiancode al tuo account Sentry.
