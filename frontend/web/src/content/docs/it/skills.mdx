...
Le skill degli agenti permettono a Tiancode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Tiancode cerca in queste posizioni:

- Config di progetto: `.tiancode/skills/<name>/SKILL.md`
- Config globale: `~/.config/tiancode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Tiancode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.tiancode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/tiancode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.tiancode/skills/git-release/SKILL.md` cosi':
...
Tiancode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `tiancode.json`:
...
**Per agenti integrati** (in `tiancode.json`):
