...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Tiancode include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Tiancode.
...
:::note
Questo strumento e' disponibile solo quando `TIANCODE_EXPERIMENTAL_LSP_TOOL=true` (oppure `TIANCODE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Tiancode o quando la variabile d'ambiente `TIANCODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Tiancode:
