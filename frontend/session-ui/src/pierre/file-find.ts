...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("tiancode-find")
  api.delete("tiancode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("tiancode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("tiancode-find", new Highlight(...rest))
    return true
  }
