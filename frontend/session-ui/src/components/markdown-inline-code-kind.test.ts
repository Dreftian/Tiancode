...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@tiancode-ai/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/tiancode`)).toBe("path")
    expect(inlineCodeKind(`@tiancode-ai/app`)).toBe("path")
  ...
  test("detects urls", () => {
    ...
    expect(inlineCodeKind(`file:///tmp/tiancode`)).toBeUndefined()
