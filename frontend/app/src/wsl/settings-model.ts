...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-tiancode" | "add" | null
  loading: boolean
  width: string | null
}
...
export function addServerViewModel(input: {
  ...
  const tiancodeCheck = selectedDistro ? (state?.tiancodeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.tiancodeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.tiancodeMissing" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  tiancodeCheck: WslOpencodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingOpencode =
    input.state?.job?.kind === "install-tiancode" && input.state.job.distro === input.selectedDistro
  ...
  if (!addServerOpencodeReady(input.tiancodeCheck)) {
    const update = !!input.tiancodeCheck?.resolvedPath && input.tiancodeCheck.matchesDesktop === false
    return {
      variant: "neutral",
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.tiancodeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.tiancodeChecks[item.name]) return [`tiancode:${item.name}`]
    return []
  })
