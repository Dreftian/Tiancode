...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Tiancode needs attention", () => {
    ...
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/tiancode",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when Tiancode is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        tiancodeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.tiancode/bin/tiancode",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "tiancode is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.primaryButton.action).toBe("install-tiancode")
  })
