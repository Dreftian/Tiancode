...
import type { useServerManagementController } from "@/components/dialogs/dialog-select-server"
    ...
    <Show when={api}>
      ...
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.tiancodeChecks[item.config.distro]
          const tiancodeAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-tiancode" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={tiancodeAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installTiancode(item.config.distro))}
                    >
