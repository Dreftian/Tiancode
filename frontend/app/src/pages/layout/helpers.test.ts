...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("tiancode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("tiancode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("tiancode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("tiancode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("tiancode://open-project")).toBeUndefined()
    expect(parseDeepLink("tiancode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "tiancode://open-project?directory=/a",
      "tiancode://other?directory=/b",
      "tiancode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("tiancode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("tiancode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("tiancode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("tiancode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "tiancode://new-session?directory=/a",
      "tiancode://open-project?directory=/b",
      "tiancode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __TIANCODE__: {
        deepLinks: ["tiancode://open-project?directory=/a"],
      },
    } as unknown as Window & { __TIANCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["tiancode://open-project?directory=/a"])
