...
import type {
  ...
  ReferenceListOutput,
  SessionApi,
} from "@tiancode-ai/client/promise"
...
import { NormalizedProviderListResponse } from "@tiancode-ai/session-ui/context"
...
// Reiniciar el sidecar (lo que pasa al instalar una actualización) tarda bastante más que el
// presupuesto por defecto de retry (~1,5 s), así que el arranque insiste unos 12 s antes de dar
// un fallo por definitivo.
const BOOTSTRAP_RETRY = { attempts: 6, delay: 400 }
...
export const loadGlobalConfigQuery = (scope: ServerScope, sdk: OpencodeClient, protocol?: Promise<ServerProtocol>) =>
  queryOptions({
    ...
    queryFn: async () => {
      ...
      return retry(() => sdk.global.config.get().then((x) => x.data!), BOOTSTRAP_RETRY)
    },
  })
      ...
      retry(
        () =>
          api.list().then((projects) => {
            return projects
              .filter((p) => !!p?.id)
              .filter((p) => !!p.worktree && !p.worktree.includes("tiancode-test"))
...
function warmSessions(input: {
  ...
  return Promise.all(
    ids.map((sessionID) =>
      retry(() => input.api.get({ sessionID }), BOOTSTRAP_RETRY).then((session) =>
...
export const loadPathQuery = (
  ...
  queryOptions<Path>({
    ...
    queryFn: async () => {
      ...
      return retry(
        () => sdk.path.get({ directory: directory ?? undefined }).then((result) => result.data!),
        BOOTSTRAP_RETRY,
      )
    },
  })
...
export async function bootstrapDirectory(input: {
  ...
  // Con el servidor caído un error no le dice nada al usuario: el stream SSE se reconecta solo y
  // server-sync vuelve a pedir todo al recuperarse. Solo avisamos si la conexión sigue en pie, que
  // es cuando el fallo es del servidor y no de la caída. Sin `online` se mantiene el aviso de
  // siempre.
  const connected = () => input.online?.() !== false
  ...
  ;(async () => {
    const slow = [
      ...
      !seededProject &&
        (() =>
          retry(() => input.api.project.current({ location: { directory: input.directory } }), BOOTSTRAP_RETRY).then(
            (project) => input.setStore("project", project.id),
          )),
      !seededPath &&
        (() =>
          input.queryClient
