  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://tiancode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://tiancode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://tiancode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://tiancode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://tiancode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://tiancode.example.com"),
      ),
    ).toEqual({
