...
describe("buildRequestParts", () => {
    ...
    expect(
      result.requestParts.some(
        (part) =>
          part.type === "text" &&
          part.synthetic &&
          part.metadata?.tiancodeComment &&
          (part.metadata.tiancodeComment as { comment?: string }).comment === "check this",
      ),
    ).toBe(true)
      ...
      images: [
        {
          type: "image",
          id: "img_external",
          filename: "tiancode.global.dat",
          sourcePath: "C:\\Users\\Luke\\AppData\\Roaming\\ai.tiancode.desktop.beta\\tiancode.global.dat",
          mime: "text/plain",
  ...
  test("handles macOS paths correctly", () => {
    ...
    const result = buildRequestParts({
      ...
      messageID: "msg_mac_1",
      sessionID: "ses_mac_1",
      sessionDirectory: "/Users/kelvin/Projects/tiancode",
    })
    ...
    if (filePart?.type === "file") {
      ...
      expect(() => new URL(filePart.url)).not.toThrow()
      // Should be a normal Unix path
      expect(filePart.url).toBe("file:///Users/kelvin/Projects/tiancode/README.md")
    }
  })
