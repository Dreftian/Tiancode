import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@tiancode-ai/client/promise"
...
import { List, type ListRef } from "@tiancode-ai/ui/list"
import { ProviderIcon } from "@tiancode-ai/ui/provider-icon"
import { Spinner } from "@tiancode-ai/ui/spinner"
import { Tag } from "@tiancode-ai/ui/tag"
import { TextField } from "@tiancode-ai/ui/text-field"
import { ButtonV2 } from "@tiancode-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@tiancode-ai/ui/v2/dialog-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "tiancode-go") return language.t("dialog.provider.tiancodeGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "tiancode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.tiancode.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["tiancode", "tiancode-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "tiancode" || provider.id === "tiancode-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "tiancode"
                                ? "dialog.provider.tiancode.tagline"
                                : "dialog.provider.tiancodeGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "tiancode"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.tiancodeZen.line1")}</div>
              <div>{language.t("provider.connect.tiancodeZen.line2")}</div>
              <div>
                {language.t("provider.connect.tiancodeZen.visit.prefix")}
                <ExternalLink
                  href="https://tiancode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.tiancodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.tiancodeZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "tiancode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.tiancodeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.tiancodeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.tiancodeZen.visit.prefix")}
                <ExternalLink href="https://tiancode.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.tiancodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.tiancodeZen.visit.suffix")}
              </div>
