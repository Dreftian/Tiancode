...
- `tiancode dev web` proxies `https://app.opencode.ai`, so local UI/CSS changes will not show there.
- For local UI changes, run the backend and app dev servers separately.
- Backend (from `backend/tiancode`): `bun run --conditions=browser ./src/index.ts serve --port 4096`
- App (from `backend/app`): `bun dev -- --port 4444`
