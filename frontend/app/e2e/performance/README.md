...
The app's high-volume performance diagnostics live under `frontend/app/e2e/performance` and are excluded from normal local and CI Playwright discovery. The benchmark config builds the app and serves the production bundle before running scenarios serially.

Run the suite explicitly from `backend/app`:
...
Set `TIANCODE_PERFORMANCE_TRACE_DIR` to emit a standard Chrome DevTools trace for every benchmark page automatically:

```sh
TIANCODE_PERFORMANCE_TRACE_DIR=/tmp/tiancode-performance-traces \
...
INP analysis requires a trace with a supported navigation/interaction insight. Selector statistics require a trace captured with `TIANCODE_PERFORMANCE_SELECTOR_TRACE=1`.
