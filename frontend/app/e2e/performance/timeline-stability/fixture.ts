...
import { SessionStatusEvent } from "@tiancode-ai/schema/session-status-event"
import { SessionV1 } from "@tiancode-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Tiancode/TimelineStability"
...
export const model = { providerID: "tiancode", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("tiancode.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
