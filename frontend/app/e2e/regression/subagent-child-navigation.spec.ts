...
import { currentSession, mockTiancodeServer } from "../utils/mock-server"
import { expectSessionTitle } from "../utils/waits"

const directory = "C:/Tiancode/SubagentNavigation"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#tiancode-titlebar-right")
...
async function setup(page: Page, events?: () => EventPayload[]) {
  await mockTiancodeServer(page, {
    ...
    provider: {
      ...
      connected: ["tiancode"],
      default: { providerID: "tiancode", modelID: "claude-opus-4-6" },
    },
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "tiancode", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem(
        "tiancode.global.dat:server",
        JSON.stringify({
          projects: { local: [{ worktree: directory, expanded: true }] },
          lastProject: { local: directory },
        }),
      )
      localStorage.setItem("tiancode.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
