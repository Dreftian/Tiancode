    ...
    const connect = () => {
      ...
      current.addEventListener("open", () => {
        ...
        request("initialize", {
          ...
          capabilities: {},
          clientInfo: { name: "tiancode", version: "0.0.0" },
        })
      })
...
function openEditorSocket(connection: EditorConnection, WebSocketImpl: typeof WebSocket) {
  ...
  // Bun's WebSocket accepts a headers init option at runtime; the DOM typings omit it.
  const options = { headers: { "x-claude-code-ide-authorization": connection.authToken } } as {
    headers?: Record<string, string>
  }
  return new WebSocketImpl(connection.url, options as never)
}
