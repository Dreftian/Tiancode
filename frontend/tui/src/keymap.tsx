...
export const TIANCODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const TIANCODE_MODE_KEY = "tiancode.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(TIANCODE_MODE_KEY, TIANCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(TIANCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(TIANCODE_MODE_KEY, stack.at(-1)?.mode ?? TIANCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? TIANCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(TIANCODE_MODE_KEY, undefined)
