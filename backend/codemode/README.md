...
{
  "dependencies": {
    "@tiancode-ai/codemode": "workspace:*"
  }
}
```
...
```ts
import { CodeMode, Tool } from "@tiancode-ai/codemode"
...
```ts
import { CodeMode, OpenAPI } from "@tiancode-ai/codemode"
...
const runtime = CodeMode.make({ tools: { tiancode: api.tools } })
...
The catalog-entry budget defaults to 2,000 estimated tokens (characters / 4, the same heuristic Tiancode uses). It applies only to full tool entries shown in the catalog; fixed instructions and namespace summaries are not counted. Override it when constructing a runtime:
...
```ts
import { toolError } from "@tiancode-ai/codemode"
