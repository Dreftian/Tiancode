...
This is the living design and status document for `@tiancode-ai/codemode` and its existing V2 Tiancode adapter.
...
`backend/tiancode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
