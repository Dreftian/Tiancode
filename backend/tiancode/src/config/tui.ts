...
import { isRecord } from "@tiancode-ai/tui/util/record"
...
import { TuiKeybind } from "@tiancode-ai/tui/config/keybind"
...
import { TuiConfig } from "@tiancode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@tiancode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old tiancode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.tiancode`
  // folders between cwd and home, and TIANCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.TIANCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit TIANCODE_TUI_CONFIG override, if set.
  if (Flag.TIANCODE_TUI_CONFIG) {
    const configFile = Flag.TIANCODE_TUI_CONFIG
  ...
  // 4. `.tiancode` directories (and TIANCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".tiancode") || dir === Flag.TIANCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".tiancode") && dir !== Flag.TIANCODE_CONFIG_DIR) continue
