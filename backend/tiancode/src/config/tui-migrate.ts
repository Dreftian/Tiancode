...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from tiancode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const tiancode = await tiancodeFiles(input)
  for (const file of tiancode) {
...
async function tiancodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "tiancode"),
    ...(await Filesystem.findUp(["tiancode.json", "tiancode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "tiancode"))
  }
  if (Flag.TIANCODE_CONFIG) files.push(Flag.TIANCODE_CONFIG)
