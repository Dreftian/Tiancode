...
import { IdeEvent } from "@tiancode-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["TIANCODE_CALLER"] === "vscode" || process.env["TIANCODE_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.tiancode"], {
    nothrow: true,
  })
