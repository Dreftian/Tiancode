...
import { buildPrompt } from "@tiancode-ai/core/session/compaction"
import { SessionCompactionEvent } from "@tiancode-ai/schema/session-compaction-event"
...
export class Service extends Context.Service<Service, Interface>()("@tiancode/SessionCompaction") {}
  ...
  Effect.gen(function* () {
    ...
    const select = Effect.fn("SessionCompaction.select")(function* (input: {
      ...
      for (let i = recent.length - 1; i >= 0; i--) {
        const turn = recent[i]!
        // Estimated here rather than upfront: the loop stops once the budget is spent, so
        // sizing every candidate turn does work that is then discarded.
