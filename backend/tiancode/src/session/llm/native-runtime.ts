...
import type { LLMClientShape } from "@tiancode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("tiancode"))
    return { type: "unsupported", reason: "provider is not openai, tiancode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @tiancode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains tiancode-owned. The native runtime only adapts
      // the @tiancode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
