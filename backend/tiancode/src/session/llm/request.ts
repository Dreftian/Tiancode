...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  ...
  const system = [
    [
      ...(input.lightweight
        ? SystemPrompt.local(input.model)
        : input.agent.prompt
          ? [input.agent.prompt]
  ...
  const tiancodeProjectID = input.model.providerID.startsWith("tiancode")
    ...
    headers: {
      ...(input.model.providerID.startsWith("tiancode")
        ? {
            ...
            "x-tiancode-session": input.sessionID,
            "x-tiancode-request": input.user.id,
            "x-tiancode-client": input.flags.client,
