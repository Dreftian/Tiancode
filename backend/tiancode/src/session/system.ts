  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      environment: Effect.fn("SystemPrompt.environment")(function* (model: Provider.Model) {
        ...
        return [
          [
            `You are powered by the model named "${model.name}" (provider ${model.providerID}). The exact model ID is ${model.providerID}/${model.api.id}. When asked which model you are, answer with the display name "${model.name}" and its exact ID.`,
