...
import type { AssistantMessage, Message, OpencodeClient, Session, SessionMessageResponse } from "@tiancode-ai/sdk/v2"
...
export const AuthMethodID = "tiancode-login"
...
export class Service extends Context.Service<Service, Interface>()("@tiancode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `tiancode auth login` in the terminal",
      name: "Login with tiancode",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, tiancode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const tiancodeProvider = providers[ProviderV2.ID.make("tiancode")]
  const tiancodeModel = tiancodeProvider ? Provider.sort(Object.values(tiancodeProvider.models))[0] : undefined
  if (tiancodeProvider && tiancodeModel) return { providerID: tiancodeProvider.id, modelID: tiancodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Tiancode service failure", service })
}
