...
export const DEPRECATED_PLUGIN_PACKAGES = ["tiancode-openai-codex-auth", "tiancode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, tiancodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(tiancodeVersion) || semver.major(tiancodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.tiancode
  if (typeof range !== "string") return
  if (!semver.satisfies(tiancodeVersion, range)) {
    throw new Error(`Plugin requires tiancode ${range} but running ${tiancodeVersion}`)
  }
}
