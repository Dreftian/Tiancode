...
import { OauthCallbackPage } from "@tiancode-ai/core/oauth/page"
...
export async function CodexAuthPlugin(input: PluginInput, options: CodexAuthPluginOptions = {}): Promise<Hooks> {
  ...
  return {
        ...
        return Object.fromEntries(
          Object.entries(provider.models)
            .filter(([, model]) => {
              ...
              // parseFloat("5.10") is 5.1, so a future gpt-5.10 would read as older than 5.4 and be
              // dropped from the list. Compare the parts as integers instead.
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "tiancode"
      output.headers["User-Agent"] = `tiancode/${InstallationVersion} (${os.platform()} ${os.release()}; ${os.arch()})`
