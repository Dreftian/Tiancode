...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@tiancode-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@tiancode-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@tiancode-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@tiancode-ai/tui/context/theme"
...
import { createCommandShim } from "@tiancode-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@tiancode-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".tiancode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".tiancode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".tiancode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.TIANCODE_PURE ? [] : pluginOrigins
    if (Flag.TIANCODE_PURE && pluginOrigins.length) {
    }
