...
import { TiancodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for tiancode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(TiancodeHttpApi)
...
export const PublicApi = TiancodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "tiancode",
    version: "1.0.0",
    description: "tiancode api",
