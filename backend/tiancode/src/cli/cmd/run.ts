import type { PermissionV1 } from "@tiancode-ai/core/v1/permission"
import { FSUtil } from "@tiancode-ai/core/fs-util"
// CLI entry point for `tiancode run` and `tiancode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`tiancode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`tiancode --mini --attach`): connects to a running
//      tiancode server and runs interactive mode against it.
//
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run tiancode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running tiancode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to TIANCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to TIANCODE_SERVER_USERNAME or 'tiancode')",
      })
      .option("dir", {
        type: "string",
    ...
    yield* Effect.promise(async () => {
      ...
      async function execute(sdk: OpencodeClient) {
        ...
        async function loop(client: OpencodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
          ...
          // A subagent runs in a child session with its own id, so tracking only the root session
          // left its permission requests unanswered and the run hung waiting for a reply.
      ...
      const sdk = createOpencodeClient({
        baseUrl: "http://tiancode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
