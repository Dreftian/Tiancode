...
import type { NamedError } from "@tiancode-ai/core/util/error"
...
describe("session.retry.retryable", () => {
  ...
  test("maps free limits to Go upsell action", () => {
    ...
    expect(SessionRetry.retryable(error, "tiancode")).toEqual({
  ...
  test("maps Go subscription limits to workspace PAYG upsell", () => {
    ...
    expect(SessionRetry.retryable(error, "tiancode-go")).toEqual({
      message:
  ...
  test("maps Go subscription limits without limit metadata", () => {
    ...
    expect(SessionRetry.retryable(error, "tiancode-go")?.action?.message).toBe(
