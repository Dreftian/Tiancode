...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.TIANCODE_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.TIANCODE_TEST_HOME = prev
      }),
  )
...
describe("skill", () => {
  ...
  it.live("discovers skills from .tiancode/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".tiancode", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
...
`,
          ...
          const skill = yield* Skill.Service
          const list = (yield* skill.all()).filter((s) => !s.location.startsWith("<built-in"))
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".tiancode", "skill", "dir-skill", "SKILL.md"),
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".tiancode", "skill", "dir-skill"))
  ...
  it.live("discovers multiple skills from .tiancode/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".tiancode", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".tiancode", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".tiancode", "skill", "no-frontmatter", "SKILL.md"),
              `# No Frontmatter
...
`,
          ...
          const skill = yield* Skill.Service
          expect((yield* skill.all()).filter((s) => !s.location.startsWith("<built-in"))).toEqual([])
        }),
      { git: true },
    ),
  )
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".tiancode", "skill", "manual-skill", "SKILL.md"),
              `---
name: manual-skill
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".tiancode", "skill", "tiancode-skill", "SKILL.md"),
                `---
name: tiancode-skill
description: A skill in the .tiancode/skill directory.
---
...
`,
          ...
          const skill = yield* Skill.Service
          const list = (yield* skill.all()).filter((s) => !s.location.startsWith("<built-in"))
          expect(list.map((s) => s.name)).toEqual(["tiancode-skill"])
        }),
      { git: true },
    ),
  )
...
`,
              ...
              Bun.write(
                path.join(dir, ".tiancode", "skill", "agent-skill", "SKILL.md"),
                `---
name: tiancode-skill
description: A skill in the .tiancode/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".tiancode", "skills", "agent-skill", "SKILL.md"),
                `---
name: tiancode-skill
description: A skill in the .tiancode/skills directory.
---
