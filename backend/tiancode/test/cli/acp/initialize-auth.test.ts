...
describe("tiancode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ tiancode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ tiancode }))
        ...
        expect(initialized.agentInfo?.name).toBe("Tiancode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ tiancode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ tiancode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("tiancode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "tiancode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.TIANCODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ tiancode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("tiancode-login")
