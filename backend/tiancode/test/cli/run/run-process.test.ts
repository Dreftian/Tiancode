// Subprocess integration tests for `tiancode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `tiancode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `TIANCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("tiancode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, tiancode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* tiancode.run("say hi")
  ...
  cliIt.concurrent(
    ...
    ({ llm, tiancode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* tiancode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, tiancode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* tiancode.run("think", { extraArgs: ["--thinking"] })
        tiancode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* tiancode.run("think again")
        tiancode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, tiancode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* tiancode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, tiancode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* tiancode.run("say hi", { format: "json" })
        tiancode.expectExit(result, 0)

        const events = tiancode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ tiancode }) =>
      Effect.gen(function* () {
        const result = yield* tiancode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, tiancode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* tiancode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, tiancode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* tiancode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, tiancode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* tiancode.run("request permission", { permission: { bash: "ask" } })
        tiancode.expectExit(denied, 0)
        ...
        const allowed = yield* tiancode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        tiancode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* tiancode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        tiancode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, tiancode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* tiancode.serve()

        const result = yield* tiancode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, tiancode }) =>
      Effect.gen(function* () {
        const result = yield* tiancode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, tiancode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* tiancode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
