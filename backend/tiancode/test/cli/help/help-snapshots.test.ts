...
// terminal sizes. The default tiancode tui command is excluded —
// `tiancode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `tiancode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real tiancode command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("tiancode CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ tiancode }) =>
      Effect.gen(function* () {
        const topLevel = yield* tiancode.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* tiancode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`tiancode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`tiancode ${argv.join(" ")} --help`)
        }
