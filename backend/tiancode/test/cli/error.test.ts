...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/tiancode.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/tiancode.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/tiancode.jsonc", dir: ".tiancode", suggestion: "tiancode" },
        expected:
          'Directory ".tiancode" in /tmp/tiancode.jsonc is not valid. Rename the directory to "tiancode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/tiancode.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/tiancode.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `tiancode models` to list available models",
      "Or check your config (tiancode.json) provider/model names",
    ].join("\n")
