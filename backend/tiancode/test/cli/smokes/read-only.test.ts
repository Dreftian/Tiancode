...
describe("tiancode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // TIANCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ tiancode }) =>
      Effect.gen(function* () {
        const r = yield* tiancode.spawn(["mcp", "list"])
        tiancode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ tiancode }) =>
      Effect.gen(function* () {
        const r = yield* tiancode.spawn(["providers", "list"])
        tiancode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ tiancode }) =>
      Effect.gen(function* () {
        const r = yield* tiancode.spawn(["models"])
        tiancode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ tiancode }) =>
      Effect.gen(function* () {
        const r = yield* tiancode.spawn(["agent", "list"])
        tiancode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh TIANCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ tiancode }) =>
      Effect.gen(function* () {
        const r = yield* tiancode.spawn(["session", "list"])
        tiancode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ tiancode }) =>
      Effect.gen(function* () {
        const r = yield* tiancode.spawn(["stats"])
        tiancode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under TIANCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ tiancode }) =>
      Effect.gen(function* () {
        const r = yield* tiancode.spawn(["db", "path"])
        tiancode.expectExit(r, 0, "db path")
