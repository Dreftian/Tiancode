...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ tiancode }) =>
    Effect.gen(function* () {
      const result = yield* tiancode.spawn(["--replay-limit", "10"])

      tiancode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ tiancode }) =>
    Effect.gen(function* () {
      const result = yield* tiancode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ tiancode }) =>
    Effect.gen(function* () {
      const result = yield* tiancode.spawn(["--mini", "--port", "4096"])
