// Subprocess test harness for the tiancode CLI. Spawns the real binary against
...
//
// Configuration flows through tiancode's built-in test affordances:
//   - TIANCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - TIANCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - TIANCODE_DISABLE_PROJECT_CONFIG : skip walking up for tiancode.json
//   - TIANCODE_PURE                : skip external plugin discovery + install
//   - TIANCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `tiancode.run` is fully wired. The shape supports adding more
// builders (`tiancode.serve(opts)`, `tiancode.acp(opts)`, `tiancode.auth(...)`)
...
const tiancodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(tiancodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    TIANCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    TIANCODE_CONFIG_CONTENT: configJson,
    TIANCODE_DISABLE_PROJECT_CONFIG: "1",
    TIANCODE_PURE: "1",
    TIANCODE_DISABLE_AUTOUPDATE: "1",
    TIANCODE_DISABLE_AUTOCOMPACT: "1",
    TIANCODE_DISABLE_MODELS_FETCH: "1",
    TIANCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `tiancode run`. New flags should
...
// `tiancode serve` is a long-lived process — it never exits on its own.
...
// `tiancode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `tiancode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `tiancode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("tiancode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          TIANCODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("tiancode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("tiancode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "tiancode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `tiancode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("tiancode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const tiancode: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, tiancode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `tiancode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "tiancode") {
...
// every call site. Use this for any test that needs the tiancode CLI fixture.
//
...
// resources (e.g. `tiancode.serve`) without an extra `Effect.scoped` wrapper —
