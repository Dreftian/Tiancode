...
it.instance("plan agent denies edits except .tiancode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".tiancode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".tiancode", "skill", "perm-skill")
...
`,
      ...
      process.env.TIANCODE_TEST_HOME = test.directory
