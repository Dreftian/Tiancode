...
/**
 * Scoped override for `Flag.TIANCODE_WORKSPACE_ID`. Saves the previous value
...
export function withFixedWorkspaceID(id: WorkspaceV2.ID): Effect.Effect<void, never, Scope.Scope> {
  return Effect.gen(function* () {
    const previous = Flag.TIANCODE_WORKSPACE_ID
    Flag.TIANCODE_WORKSPACE_ID = id
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        Flag.TIANCODE_WORKSPACE_ID = previous
      }),
    )
  })
}
