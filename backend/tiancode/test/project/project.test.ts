...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const tiancodeFile = path.join(tmp, ".git", "tiancode")
      expect(yield* Effect.promise(() => Bun.file(tiancodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "tiancode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "tiancode")
      const wrongCache = path.join(parentDir, ".git", "tiancode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "tiancode")
      const cacheB = path.join(bareB, "tiancode")
      const wrongCache = path.join(parentDir, ".git", "tiancode")
