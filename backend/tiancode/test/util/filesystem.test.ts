...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "tiancode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "tiancode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "tiancode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "tiancode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["tiancode.json", "tiancode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "tiancode.json"),
        path.join(project, "tiancode.jsonc"),
      ])
    })
  })
