...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips tiancode-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-tiancode-directory"]).toBeUndefined()
      expect(forwarded["x-tiancode-workspace"]).toBeUndefined()
