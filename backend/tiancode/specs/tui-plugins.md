...
- Author package entrypoint is `@tiancode-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/tiancode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@tiancode-ai/plugin/tui`.
- `@tiancode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@tiancode-ai/plugin/tui"
...
- The value is a semver range checked against the running Tiancode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.tiancode` is absent, no check is performed (backward compatible).
...
- `tiancode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `tiancode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.tiancode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.tiancode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.tiancode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.tiancode` dir or `TIANCODE_CONFIG_DIR`, Tiancode installs `@tiancode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@tiancode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Tiancode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Tiancode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Tiancode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@tiancode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"tiancode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `tiancode.default`.
...
- Local plugins persist installed themes under the local `.tiancode/themes` area near the plugin config source.
...
- `--pure` / `TIANCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.tiancode/plugins/tui-smoke.tsx`
- Local vim plugin: `.tiancode/plugins/tui-vim.tsx`
- Local smoke config: `.tiancode/tui.json`
- Local smoke theme: `.tiancode/plugins/smoke-theme.json`
