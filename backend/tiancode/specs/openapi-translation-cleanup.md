...
Trim `backend/tiancode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `backend/tiancode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `backend/tiancode`.
- `[x]` `bun typecheck` from `backend/tiancode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `backend/tiancode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `backend/tiancode`.
- `[x]` `bun dev generate > /tmp/tiancode-openapi.json` from `backend/tiancode`.
- `[x]` `./backend/sdk/js/script/build.ts` from repo root.
...
- Edit `backend/tiancode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/tiancode-openapi.json` from `backend/tiancode`.
- `./backend/sdk/js/script/build.ts` from repo root.
- Inspect generated SDK request param types before deleting each override.
- `bun typecheck` from `backend/tiancode`.

### PR 4: Move Path Parameter Patterns Into ID Schemas

- Audit `PathParameterSchemas` and `pathParameterSchema()` in `public.ts`.
- Check source schemas in files like `backend/tiancode/src/session/schema.ts`, `backend/tiancode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `backend/tiancode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `backend/tiancode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
