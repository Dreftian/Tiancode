...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve tiancode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "tiancode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "tiancode")).toBe("openai")
    expect(statProvider("big-pickle", "", "tiancode")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "tiancode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep tiancode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "tiancode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "tiancode"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep tiancode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "tiancode"), country: "US" })).toMatchObject([
