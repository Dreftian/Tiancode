...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@tiancode/acme")).toBe("@tiancode/acme")
    expect(Npm.sanitize("@tiancode/acme@1.0.0")).toBe("@tiancode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/tiancode/acme.git"
    const expected = win ? "acme@git+https_//github.com/tiancode/acme.git" : spec
