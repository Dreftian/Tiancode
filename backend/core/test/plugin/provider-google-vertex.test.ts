...
import { GoogleVertexPlugin } from "@tiancode-ai/core/plugin/provider/google-vertex"
...
describe("GoogleVertexPlugin", () => {
  it.effect("ignores OpenAI-compatible providers that are not Google Vertex", () =>
    Effect.gen(function* () {
      ...
      const provider = required(yield* catalog.provider.get(ProviderV2.ID.tiancode))
