...
import { ModelsDevPlugin } from "@tiancode-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.TIANCODE_MODELS_PATH,
          disabled: Flag.TIANCODE_DISABLE_MODELS_FETCH,
        }
        Flag.TIANCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.TIANCODE_MODELS_PATH = previous.path
          Flag.TIANCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
