...
import { ToolFailure } from "@tiancode-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@tiancode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.TIANCODE_WEBSEARCH_PROVIDER === "exa" || process.env.TIANCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.TIANCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("TIANCODE_EXPERIMENTAL") || truthy("TIANCODE_ENABLE_EXA") || truthy("TIANCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("TIANCODE_ENABLE_PARALLEL") || truthy("TIANCODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `tiancode/${InstallationVersion}`,
