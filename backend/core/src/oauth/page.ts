...
// etc.).
//
// The visual language mirrors the Tiancode app: the design tokens are a curated
// subset of the OC-2 semantic tokens in `frontend/ui/src/styles/theme.css`, and
// the wordmark is the same geometry as `frontend/ui/src/components/logo.tsx`.
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `Tiancode is now connected to ${escapeHtml(provider)}.` : "Tiancode is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `Tiancode couldn't finish connecting to ${escapeHtml(provider)}.`
        : "Tiancode couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from Tiancode.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · Tiancode</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("Tiancode couldn't finish connecting to "+PROVIDER+"."):"Tiancode couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from Tiancode."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("Tiancode is now connected to "+PROVIDER+"."):"Tiancode is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// Curated subset of OC-2 tokens (frontend/ui/src/styles/theme.css). Default is
...
// Tiancode wordmark — same path geometry as frontend/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="Tiancode" role="img">
