...
import { Catalog } from "@tiancode-ai/schema/catalog"
...
export class Service extends Context.Service<Service, Interface>()("@tiancode/v2/Catalog") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const available = (provider: ProviderV2.Info, integration: Integration.Info | undefined) => {
      ...
      if (
        provider.id === "local" ||
        provider.id === "tiancode-native" ||
        provider.id === "ollama" ||
        provider.id === "lmstudio"
      ) {
        return true
      }
      if (provider.api.url || typeof provider.request.body.baseURL === "string") return true
    ...
    const result: Interface = {
      ...
      model: {
        ...
        small: Effect.fn("CatalogV2.model.small")(function* (providerID) {
          ...
          if (providerID === ProviderV2.ID.tiancode) {
