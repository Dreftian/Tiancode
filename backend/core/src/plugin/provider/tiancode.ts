...
import type { IntegrationOAuthMethodRegistration } from "@tiancode-ai/plugin/v2/effect/integration"
import { define } from "@tiancode-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@tiancode-ai/sdk/v2/types"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("tiancode")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Tiancode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("tiancode", (integration) => {
        integration.name = "Tiancode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "tiancode", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("tiancode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("tiancode")
      ...
      const item = catalog.provider.get(ProviderV2.ID.tiancode)
      if (!item) return
      const hasKey = Boolean(process.env.TIANCODE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("tiancode")),
