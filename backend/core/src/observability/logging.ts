...
export function fileLogger(file = path.join(Global.Path.log, "tiancode.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.TIANCODE_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.TIANCODE_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
