...
const snapshot = path.join(root, "backend/core/schema.json")
const tsDir = path.join(root, "backend/core/src/database/migration")
const registry = path.join(root, "backend/core/src/database/migration.gen.ts")
const schema = path.join(root, "backend/core/src/database/schema.gen.ts")
...
async function generate() {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "tiancode-core-migration-"))
...
async function check() {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "tiancode-core-migration-check-"))
  const incremental = path.join(temporary, "incremental")
  const full = path.join(temporary, "full")
  try {
    await fs.mkdir(incremental)
...
async function drizzle(temporary: string, output: string, name?: string) {
  ...
  await Bun.write(
    config,
    `import config from ${JSON.stringify(pathToFileURL(path.join(root, "backend/core/drizzle.config.ts")).href)}

export default { ...config, out: ${JSON.stringify(output)} }
`,
  )
  await $`bun drizzle-kit generate --config ${config} ${name ? ["--name", name] : []}`.cwd(
    path.join(root, "backend/core"),
  )
}
