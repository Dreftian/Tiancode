...
9. Run the focused test file and `bun typecheck` from `backend/opencode`.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep\|withTestInstance" -- backend/opencode/test
```
