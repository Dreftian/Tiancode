...
Practical reference for a future `backend/server` split after the opencode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `backend/opencode`.
...
Until enough shared service code lives outside `backend/opencode`, a future
`backend/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `backend/opencode`

It should not import `backend/opencode` services while `backend/opencode`
...
3. Extract pure HttpApi contract modules into `backend/server` once the contract
   can compile without importing `backend/opencode` implementation details.
