...
Delete `backend/opencode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `backend/opencode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `backend/opencode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `backend/opencode`
- TUI plugin loader tests in `backend/opencode` if runtime plugin API wiring changed
