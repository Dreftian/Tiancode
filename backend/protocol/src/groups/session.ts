...
import { PromptInput } from "@tiancode-ai/schema/prompt-input"
import { Session } from "@tiancode-ai/schema/session"
import { Project } from "@tiancode-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@tiancode-ai/schema/schema"
...
import { SessionEvent } from "@tiancode-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Tiancode process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Tiancode process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
