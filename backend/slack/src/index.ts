...
import { createOpencode, type ToolPart } from "@tiancode-ai/sdk"
...
console.log("🚀 Starting tiancode server...")
const tiancode = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await tiancode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new tiancode session...")
    const { client, server } = tiancode
    ...
    console.log("✅ Created tiancode session:", createResult.data.id)
  ...
  console.log("📝 Sending to tiancode:", message.text)
