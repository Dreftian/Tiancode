...
Schema-first LLM core for tiancode. One typed request, response, event, and tool language; provider quirks live in adapters, not in calling code.

```ts
import { Effect } from "effect"
import { LLM, LLMClient } from "@tiancode-ai/llm"
...
```ts
import { OpenAI, CloudflareAIGateway } from "@tiancode-ai/llm/providers"
