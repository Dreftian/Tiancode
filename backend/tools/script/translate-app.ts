...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "tiancode/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".tiancode/glossary/zh-cn.md"
  if (locale === "zht") return ".tiancode/glossary/zh-tw.md"
  return `.tiancode/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Tiancode did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Tiancode returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Tiancode session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Tiancode runs for 'all' (default: 4)
      --model <provider/id>  Tiancode model (default: tiancode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Tiancode
...
`)
  ...
  if (failed.length) console.error(`\nTiancode failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.TIANCODE_DISABLE_PROJECT_CONFIG = "1"
  env.TIANCODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["tiancode", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.TIANCODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["tiancode", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.TIANCODE_CONFIG
  delete env.TIANCODE_CONFIG_DIR
  delete env.TIANCODE_CONFIG_CONTENT
  delete env.TIANCODE_PERMISSION
  delete env.TIANCODE_AUTO_SHARE
  return env
}
