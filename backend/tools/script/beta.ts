...
const model = "tiancode/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("backend/tiancode")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with tiancode...`)
  ...
  try {
    await $`tiancode run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  tiancode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with tiancode")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with tiancode...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `backend/tiancode`.",
