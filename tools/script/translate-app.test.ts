...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".tiancode/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".tiancode/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".tiancode/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "tiancode/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
...
{"variants":{}}
`
    expect(modelVariants(output, "tiancode/gpt-5.5")).toEqual({
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "tiancode/gpt-5.5", ["frontend/app/src/i18n/fr.ts"])
