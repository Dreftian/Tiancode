...
import { createOpencode } from "@tiancode-ai/sdk"
...
`)
  ...
  const tiancode = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await tiancode.client.session.create()
    const result = await tiancode.client.session
      .prompt({
