...
- Use `testEffect(...)` from `backend/opencode/test/lib/effect.ts` for tests that exercise Effect services, layers, runtime context, scoped resources, or platform integrations.
...
- Run tests from package directories such as `backend/opencode`; never run package tests from the repo root.
