...
let
  packageJson = lib.pipe ../backend/tiancode/package.json [
    builtins.readFile
    builtins.fromJSON
  ];
...
stdenvNoCC.mkDerivation {
  ...
  src = lib.fileset.toSource {
    ...
    fileset = lib.fileset.intersection (lib.fileset.fromSource (lib.sources.cleanSource ../.)) (
      lib.fileset.unions [
        ...
        ../.github/TEAM_MEMBERS # required by @tiancode-ai/script
      ]
    );
  };
  ...
  buildPhase = ''
    ...
    bun install \
      ...
      --os="${bunOs}" \
      --filter '!./' \
      --filter './backend/tiancode' \
      --filter './backend/desktop' \
      --filter './backend/app' \
      --frozen-lockfile \
      --ignore-scripts \
      --no-progress
